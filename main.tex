...
Furthermore, since $G$ is $3$-regular, $G_2$ has even number of vertices, and in any $3$-edge-coloring of
$G_2+B$, each vertex of $G_2$ is incident with exactly one edge colored by any color $c\in\{1,2,3\}$.
Consequently, in any such coloring, $c$ appears on even number of edges of $B$.  Hence, $S_2$ may
...
Let us formulate the consistency condition more precisely.  For $\psi\in S_2$ and distinct colors $c_1,c_2\in \{1,2,3\}$, a \emph{$(\psi,c_1,c_2)$-connector}
is a union $H$ of paths with ending edges equal to the edges of $B$ to that $\psi$ gives color $c_1$ or $c_2$ such that $G_1+H$ is a planar graph.
A \emph{subconnector} $H'$ of $H$ is any union of components of $H$.  Let $\psi_{H'}$ denote the coloring obtained from $\psi$ by exchanging colors $c_1$ and $c_2$
on the edges of $B\cap E(H')$.  Let
$$\psi\uparrow H=\{\psi_{H'}:\text{$H'$ is a subconnector of $H$}\}.$$
We now have the following.  For every $\psi\in S_2$ and each pair of distinct colors $c_1,c_2\in \{1,2,3\}$, there exists a
$(\psi,c_1,c_2)$-connector $H$ such that $\psi\uparrow H\subseteq S_2$.

We can use this property to exclude membership of some colorings from $S_2$.
For example, suppose that the coloring $\psi=111111$ belongs to $S_2$.  Due to planarity restriction, there are only five $(\psi,1,2)$-connectors;
for each of them, we list just the ending edges of the paths:
\begin{align*}
H_1&: e_1e_2, e_3e_4, e_5e_6\\
H_2&: e_1e_2, e_3e_6, e_4e_5\\
H_3&: e_1e_4, e_2e_3, e_5e_6\\
H_4&: e_1e_6, e_2e_3, e_4e_5\\
H_5&: e_1e_6, e_2e_5, e_3e_4\\
\end{align*}
Switching on all possible subsets of their components, we conclude that the sets $\psi\uparrow H_i$ for $i\in\{1,\ldots, 5\}$ contain the following colorings, up to permutation of colors
(let us remark that it suffices to try the switches involving at most half of the components, as switching on the complementary subconnector is equivalent to switching on the current
subconnector and afterwards echanging the colors $1$ and $2$):
\begin{align*}
\psi\uparrow H_1&: 111111,112222,112211,111122\\
\psi\uparrow H_2&: 111111,112222,111221,112112\\
\psi\uparrow H_3&: 111111,122122,122111,111122\\
\psi\uparrow H_4&: 111111,122221,122111,111221\\
\psi\uparrow H_5&: 111111,122221,121121,112211
\end{align*}
However, each of these sets contains a coloring not belonging to $S'_2$ and thus also not belonging to $S_2$, which contradicts the consistency condition.
...
For example, the reason we could not show that $\psi=112233$ does not belong to $S_2$ is as follows.  By planarity, there are two possible $(\psi,1,2)$-connectors.
\begin{align*}
H_1&: e_1e_2, e_3e_4\\
H_2&: e_1e_4, e_2e_3
\end{align*}
For these, we have
\begin{align*}
\psi\uparrow H_1&: 112233,111122\\
\psi\uparrow H_2&: 112233,121233\\
\end{align*}
And while $\psi\uparrow H_2$ contains a coloring not belonging to $S'_2$, both colorings in $\psi\uparrow H_1$ belong to $S'_2$, and this suffices to satisfy the consistency condition.
Similarly, Kempe chains in colors $1$ and $3$ and in colors $2$ and $3$ do not help, either.

However, both $\psi\uparrow H_1$ and $\psi\uparrow H_2$ contain colorings not belonging to $S''_2$! Hence, they both contain a coloring not belonging to $S_2$, and by the consistency
condition we conclude that the coloring $112233$ cannot belong to $S_2$.  Going over all colorings in $S''_2$, we similarly concude that all colorings in $S_2$ are among the following ones,
...
\subsubsection{Block count reducibility}

Finally, let us describe a linear programming-based strengthening of the method of consistent sets,
called \emph{block count reducibility}.
...

\section{Recoloring and the number of colorings}

% Exponentially many colorings of planar graphs
