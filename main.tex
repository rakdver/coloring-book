...
\item[\textrm{(b)}] A $10$-cycle of vertices of degree three, vertex-disjoint with $C$.
...
\subsection{The trick without precoloring}

There is another largely equivalent way of dealing with the short separating cycles:
...
or a $10$-cycle of vertices of degree three, or a clean tetrad.
\end{lemma}
\begin{proof}
If $G$ did not contain a triangle, then Euler's formula easily implies that $G$ contains a vertex of degree at most two.
...
Let $C_8$ be a triangle in $G$ such that the open disk $\Lambda_8$ bounded by $C_8$ contains a vertex of $G$,
chosen among cycles with this property so that the number of vertices of $G$ drawn in $\Lambda_8$ is minimum,
and let $G_8$ be the subgraph of $G$ drawn in the closure of $\Lambda_8$.
For $\ell=9,\ldots,12$ in order, let $C_\ell$ be a cycle in $G_{\ell-1}$ of length at most $\ell$
such that the open disk $\Lambda_\ell$ bounded by $C_\ell$ contains a vertex of $G$,
chosen among cycles with this property so that the number of vertices of $G$ drawn in $\Lambda_\ell$ is minimum,
and let $G_\ell$ be the subgraph of $G$ drawn in the closure of $\Lambda_\ell$.

Clearly, $G_{12}$ is a $2$-connected graph and $C_{12}$ is an induced cycle in $G_{12}$.
If $G_{12}$ did not contain any of the configurations (a), (b), and (c) from Lemma~\ref{lemma:no4to8-redu}, then we would 
assign charge to the vertices and faces of $G_{12}$, perform the discharging, and obtain a contradiction in the same way as in
the proof of Theorem~\ref{thm:no4to8}.  Hence, $G_{12}$ contains one of the three configurations.  Note that vertices of $G_{12}$
not belonging to $C_{12}$ have the same degree in $G$ and in $G_{12}$, and thus if $G_{12}$ contains (a) or (b),
then $G$ contains a vertex of degree at most two or a $10$-cycle of vertices of degree three.

Therefore, we can assume that $G_{12}$ contains the configuration (c), i.e., a tetrad $v_1v_2v_3v_4$ vertex-disjoint from $C_{12}$.
Since $v_1,\ldots, v_4\not\in V(C_{12})$, the path $v_1v_2v_3v_4$ is also a tetrad in $G$.  Suppose that the tetrad is not
clean in $G$, i.e., letting $v_0$ and $y$ be as in the definition of the clean tetrad, there exists a path in $G$
from $v_0$ to $y$ of length at most $8$, and thus the path $v_0v_1v_2v_3y$ is contained in a cycle $K'$ of length at most $12$.
As in the proof of Lemma~\ref{lemma:no4to8-prop}, we argue that the open disk bounded by any such cycle $K'$ contains either $x$ or $v_4$.
Let $\Lambda'$ be the open disk bounded by $K'$.  If $\Lambda'\subseteq\Lambda_{|K'|}$, then since both edges $v_2x$ and $v_3v_4$ are contained in $\Lambda_\ell$
and exactly one of them is contained in $\Lambda'$, we have $\Lambda'\subsetneq\Lambda_{|K'|}$; however, this contradicts the choice
of $C_\ell$.  Hence, $\Lambda'\not\subseteq\Lambda_{|K'|}$.  Let $s(K')\in \{8,\ldots,|K'|\}$ be the minimum such that
$\Lambda'\not\subseteq \Lambda_{s(K')}$.

Let $K$ be a cycle in $G$ containing the path $v_0v_1v_2v_3y$, chosen so that $\ell=|K|$ is minimum and
subject to that $s=s(K)$ is maximum.  For $t\in\{s,s+1,\ldots,\ell\}$, let
$Q_t$ be the subpath of $K$ containing $v_0v_1v_2v_3y$ such that endvertices of $Q_t$ belong to $C_t$
and the rest of $Q_t$ is contained in $\Lambda_t$.

Let $F_t$ be the subgraph of $K\cup C_t$ drawn in the closure of $\Lambda_t$,
and let $\FF_t$ denote the set of faces of $F_t$ contained in $\Lambda_t$.  
No face of $\FF_t$ has length three,
as otherwise either $K$ would have a chord or $C_t$ would have a chord drawn in $\Lambda_t$, contradicting the choice
of $K$ or $C_t$, and thus these faces have length at least $9$. 
Furthermore, by the choice of $K$, the faces whose boundary contains $Q_t$
have length at least $\ell+1$.  Hence, $\sum_{f\in \FF_t} |f|\ge 2(\ell+1)+9(|\FF_t|-2)$.
On the other hand, since $K$ has at least one edge not contained in the closure
of $\Lambda_t$, we have $\sum_{f\in \FF} |f|\le |C_t|+2(\ell-1)$.
Since $|C_t|\le 12$, we conclude that $|\FF_t|=2$, and thus $K$ intersects $\Lambda_t$ only in $Q_t$.

Let $w_t$ and $z_t$ be the ends of $Q_t$ and let $P_t$ be the path in $K$
distinct from $Q_t$ joining $w_t$ with $z_t$.
Let $P'_t$ and $P''_t$ be the subpaths of $C_t$ joining $w_t$ with $z_t$, where $P'_t$ is contained in the closure
of $\Lambda$.  The cycle $Q_t\cup P''_t$ has length at least $\ell+1$, and thus
$P''_t$ is longer than $P_t$.  Let $W_t$ be the closed walk consisting of $P'_t$ and $P_t$.  Note that
$W_t$ is obtained from $C_t$ by replacing $P''_t$ with $P_t$, and thus $|W_t|<|C_t|$.  In particular, $|C_s|>|W_s|\ge 3$,
and thus $s\ge 9$.

We claim that no vertex of $G$ is contained in $\Lambda\setminus\overline{\Lambda_\ell}$.
In order to prove this, we show by induction for $t=s,\ldots,\ell$ that $\Lambda\setminus\overline{\Lambda_t}$
contains no vertex of $S$.  By the induction hypothesis, all the vertices in $\Lambda\setminus\overline{\Lambda_t}$
belong to $\overline{\Lambda_{t-1}}$ (for $t=s$, this holds since $\Lambda\subseteq\Lambda_{s-1}$ by the choice of $s$).
...

Consider any non-outer face $f$ of the graph consisting of the vertices and edges of $W$.  We have $|f|\le |W|<|C_s|$.
By the choice of $s$, we have $\Lambda\subseteq \Lambda_{s-1}$, and thus $f\subseteq\Lambda_{s-1}$.  At least one vertex
of $C_s$ is contained in the closure of $\Lambda_{s-1}$ but not in the closure of $f$, and by the minimality of $\Lambda_{s-1}$,
we conclude that no vertex of $G$ is contained in $f$.
...
\end{proof}
