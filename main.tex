...
Let $\ell_1$ denote the length of $Q_1$.
...
\end{equation}
and thus $s\le |C|-8\le 4$.  Consequently, $s\in\{0,3\}$.

Suppose first that $\max(|f_1|,\ldots,|f_4|)\le 12$.  Then the choice of $C$ implies that $V(G_C)=V(F)$, i.e., $G_C$ is obtained from $F$ by
possibly adding chords to its faces, and $G_C$ has exactly four non-outer faces of length at least 9 and all other non-outer faces of $G_C$
are triangles. ...
