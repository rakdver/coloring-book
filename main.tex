...
and $f$ is a face of $H$.  If the boundary of $f$ consists of edges and vertices
...
for an illustration.  Let us remark that we generally consider stencils $(H,f)$ such that
every vertex and edge of $H$ is incident with $f$, but we do not include this as part of the definition, since it
is sometimes convenient to speak about stencils corresponding to different faces of a single graph $H$.
...
$\GG$ is cutting-closed implies the following.
\begin{observation}
If $\GG$ is a cutting-closed hyperbolic class of graphs, then no graph in $\GG$ is drawn in the sphere.
\end{observation}
Indeed, the same argument actually shows that no graph in $\GG$ has a component drawn in a contractible subset of the surface and disjoint
...
\begin{lemma}\label{lemma:5lchyper}
...
\end{proof}
To be specific, we will be giving all applications in terms of 5-list-coloring.  However, they of course equally well
apply to any other coloring problem where the critical graphs form a hyperbolic family.

\subsection{Exponential growth and logarithmic radius}

A fundamental property of graphs from hyperbolic classes is that contractible neighborhoods grow exponentially
...
Let $G\in\GG$ be a graph with drawing in a surface $\Sigma$.
...
Let us sketch an application.  Let $G$ be a planar graph with $n$ vertices, let $L$ be an assignment of lists of size five to
vertices of $G$, and let $\psi$ be an $L$-coloring of a connected subgraph $H$ of $G$.  How to decide whether $\psi$ extends
to an $L$-coloring of $G$?  Clearly, it suffices to be able to decide whether the coloring extends to each of the subgraphs
of $G$ drawn in the faces of $H$.  Fix a face $f$ of $H$ and let $G_f$ be the subgraph of $G$ obtained by cutting along the corresponding
stencil.  If $\psi$ does not extend to an $L$-coloring of $G_f$, then $G_f$ contains a subgraph $G'$ containing $\partial G_f$
boundary-critical for $L$-coloring such that $\psi$ does not extend to an $L$-coloring of $G'$.
The graph $G'$ belongs to a hyperbolic class by Lemma~\ref{lemma:5lchyper}, and thus all its vertices are at distance $O(\log n)$ from
$\partial G_f$ by Corollary~\ref{cor:logdisk}.  Let $G''$ be the subgraph of $G_f$ induced by vertices
at distance at most $O(\log n)$ from $\partial G_f$.  As we just argued, $G''$ contains a boundary-critical subgraph to that
$\psi$ does not extend if and only if $G_f$ does, and thus $\psi$ extends to an $L$-coloring of $G_f$ if and only if it extends to
an $L$-coloring of $G''$.

Moreover, Robertson and Seymour~\cite{rs3} proved that a plane graph where every vertex is at distance at most $r$ from the boundary
of one of its faces has tree-width $O(r)$, and for an $n$-vertex graph of tree-width at most $t$, the precoloring extension problem from lists of
bounded size can be decided in time $\exp(O(t))n$.  Combining these results, we can decide whether $\psi$ extends to an $L$-coloring
of $G''$ in time $\exp(O(\log n))n=\text{poly}(n)$.  Hence, for any connected subgraph $H$ of a planar graph $G$ and
an assignment $L$ of lists of size five, we can decide whether a precoloring of $H$ extends to an $L$-coloring of $G$ in polynomial
time.  The interesting point here is that the size of $H$ is not bounded---so this is not the case that we could just check a finite
number of obstructions.  Let us also remark that the assumption that $H$ is connected is crucial; otherwise, take a plane graph $G_0$
with an assignment $L_0$ of lists of size four, add a pendant vertex adjacent to each of the vertices of $G_0$, and let $H$ be the
independent set formed by these pendant vertices.  Let $c$ be a new color not appearing in any of the lists, let
$L(v)=L_0(v)\cup\{c\}$ for every vertex $v\in V(G_0)$, and choose lists of size five containing $c$ for vertices of $H$ arbitrarily.
The precoloring of $H$ giving each vertex the color $c$ extends to an $L$-coloring of $G$ if and only of $G_0$ is $L_0$-colorable.
However, deciding colorability of planar graphs from lists of size four is NP-hard.

For surfaces without boundary, Corollary~\ref{cor:expgrowth} can clearly be used to show that the edge-width of graphs from
hyperbolic families is logarithmic in the number of their vertices.  Actually, slightly more is true.
\begin{corollary}\label{cor:logew}
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma>1$.
For every graph $G\in \GG$ drawn in a surface without boundary and every vertex $v\in V(G)$, 
there exists a non-contractible cycle in $G$ of length $O(\log |V(G)|)$ containing $v$.
\end{corollary}
\begin{proof}
Let $r=O(\log |V(G)|)$ be the minimum integer such that $\Bigl(\tfrac{\gamma}{\gamma-1}\Bigr)^r>|V(G)|$.
The conclusion of Corollary~\ref{cor:expgrowth} is then clearly false, and thus
there exists a non-contractible closed walk in $G$ containing $v$ of length at most $2r+1$.
\end{proof}
Next, we show this bound can actually be substantially improved.

\subsection{Short non-contractible cycles in hyperbolic graphs}

As a motivation, let us have a look at consequences of Corollary~\ref{cor:logew} for the projective plane and the torus.
Consider a graph $G\in \GG$ drawn in the projective plane.  By Corollary~\ref{cor:logew}, there exists a constant $c_\gamma$
depending only on the Cheeger constant $\gamma$ of $\GG$ such that $G$ contains a non-contractible
cycle $C$ of length at most $c_\gamma\log |V(G)|$.  Cutting the surface along this cycle (more precisely, along a curve
intersecting $G$ exactly in $V(C)$), we obtain a graph $G'$ with boundary-perpendicular drawing in a disk and with
$|\partial G'|\le 2|V(C)|\le 2c_\gamma\log |V(G)|$.  Since $\GG$ is cutting-closed, we have $G'\in \GG$, and
thus $|V(G)|\le |V(G')|<\gamma|\partial G'|\le 2\gamma c_\gamma\log |V(G)|$.  This is only possible if $|V(G)|$ is
small: $|V(G)|\le f^{-1}(2\gamma c_\gamma)$, where $f(x)=x/\log x$.
\begin{corollary}
For every $\gamma>1$, there exists $c$ such that if
$\GG$ is a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma$
and $G\in\GG$ is drawn in the projective plane, then $|V(G)|\le c$.
\end{corollary}
Hence, for example, 5-list-colorability of graphs drawn in the projective plane is characterized by a finite
number of obstructions.

For the torus, the situation is a bit more complicated.  Cutting along a short non-contractible cycle
splits the torus into a cylinder.   If the boundaries of the cylinder are close to one another, we can cut
along a shortest path between them, giving us again a graph drawn in a disk and concluding that $|V(G)|$ is bounded.
On the other hand, if the distance between tbe boundaries is large, we can analgously to the proof of Corollary~\ref{cor:logew}
show that $G$ contains another non-contractible cycle disjoint from the boundaries, and we can split the cylinder into two
subcylinders.  Repeating this construction, we conclude that the torus can be split into a number of subcylinders,
each of them containing $O(\log |V(G)|)$ vertices of $G$.  Actually, as we show in this section, more is true:
We can in fact obtain such a decomposition into subcylinders obtaining at most $c_\gamma$ vertices of $G$, for some $c_\gamma$
depending only on $\gamma$.  We will also give a generalization of this argument to other surfaces.

To this end, we need several definitions.
...
A set $X$ of vertices of $G$ is \emph{$d$-isolated} if the distance
between any two vertices of $X$ is greater than $2d$ and the distance from $X$ to $\partial G$ is greater than $d$.
Let $\beta_d(G)$ denote the largest size of a $d$-isolated set in $G$.
It is easy to see that a minimum-size net $(H,f)$ of a graph $G$ drawn in a fixed surface consists mostly of paths of vertices
of degree two, and thus $H$ contains a $d$-isolated set of size $\Omega(|V(H)|/d)$.  In the following lemma,
we will show that distances in $H$ are close to distances in $G$, allowing us to obtain a set of size $\Omega(|V(H)|/d)$
that is $d$-isolated in $G$.  Thus, such a net has size $|V(H)|=O(d\beta_d(G))$.  Since the neighborhoods
of vertices in a $d$-isolated set up to distance $d$ are disjoint, this will enable us to use the exponential growth guaranteed
by Corollary~\ref{cor:expgrowth} to reach the desired conclusion.

The \emph{radial multigraph} of a graph $G$ with a $2$-cell drawing in a surface $\Sigma$ is
...
Note that $R$ has a natural $2$-cell drawing in $\Sigma$, where the vertices of $V(G)$ are drawn in the same way
...
of $\Sigma$ is bounded by a cycle in $R$ in this natural drawing.  Note also that for any $x,y\in V(G)$,
the distance between $x$ and $y$ in $R$ is at most twice the distance in $G$, as a walk from $x$ to $y$ in $R$ can be obtained
from a path $P$ in $G$ by, for each $uv\in E(P)$, replacing the edge $uv$ by a path $ufv$, where $f$ is a face of $G$ incident
with $uv$.

\begin{lemma}\label{lemma:isnet}
For all integers $k,g\ge 0$ and $d\ge 1$, the following claim holds.
If $G$ is a graph with a boundary-perpendicular $2$-cell drawing in a surface $\Sigma$ of genus $g$ with $k$ holes,
then there exists a net $(H,f)$ perpendicular to $G$ such that
$$|\partial G_f|\le (8d+4)\beta_d(G)+(48d+29)(k+g)+|\partial G|.$$
\end{lemma}
\begin{proof}
The claim is trivial if $\Sigma$ is the sphere (with $H$ consisting of a single vertex) or the disk (with $H$ being
a cycle with vertex set $\partial G$).  Hence, we can assume $\Sigma$ is neither the sphere nor the disk.

Let $R$ be the radial multigraph of $G$ with its natural drawing in $\Sigma$.  
We can repeatedly delete from $R$ edges between different faces, obtaining a normal subgraph of $R$ with only one $2$-cell face.
Hence, there exists a net $(H,f)$ perpendicular to $G$ with $H\subseteq R$.  Choose such a net $H$ with $|V(H)|$ minimum.

Let $B$ be the set of vertices $y\in V(H)$ such that either $\deg_H(y)>2$ or
$y\in\partial H$.  Note that either $\Sigma$ is the projective plane, $H$ is a cycle, and $B=\emptyset$,
or every component of $H-B$ is a path.
Let $d_R$ and $d_H$ denote the distance in $R$ and in $H$, respectively.
Consider any path $Q$ in $R$ with both ends $u$ and $x$ in $V(H)$ and internal vertices in
$V(R)\setminus V(H)$.  Let $\ell$ be the length of $Q$.  We claim that if $d_H(u,B)>\ell$,
then $d_H(u,x)\le \ell$.  This is trivial if $Q$ is an edge of $H$.  Otherwise,
let $P_1$ and $P_2$ be the two edge-disjoint paths in $H$ of length $\ell$
ending in $u$, and suppose for a contradiction $x$ does not belong to either of them.
Let $f_1$ and $f_2$ be the two faces of $H\cup Q$, labelled so that the concatenation of $P_i$ with $Q$ is
contained in the boundary of $f_i$ for $i\in\{1,2\}$.  By symmetry, we can assume $u$ is incident with $f_2$
twice.  Then $(H\cup Q)-V(P_1-u)$ has only one $2$-cell face and fewer vertices than $H$, contradicting the minimality of $|V(H)|$.

More generally, we claim that for all $u,v\in V(H)$,
\begin{equation}\label{eq:fabran}
d_R(u,v)\ge \min(d_H(u,v),d_H(u,B)).
\end{equation}
Let us prove this claim by the induction on $d_R(u,v)$; the case $d_R(u,v)=0$ is trivial, and thus
we can assume $u\neq v$ and the claim holds for all vertices of $H$ whose distance in $R$ is smaller than $d_R(u,v)$.
Let $P$ be a shortest path from $u$ to $v$ in $R$, let $x$ be the first vertex on $P$ after $u$ that belongs to $H$,
let $Q$ be the segment of $P$ from $u$ to $x$, and let $\ell$ be the length of $Q$.  If $\ell\ge d_H(u,B)$,
then $d_R(u,v)\ge\ell\ge \min(d_H(u,v),d_H(u,B))$.  Hence, we can assume $d_H(u,B)>\ell$,
and by the previous paragraph, $d_H(u,x)\le \ell$.  By the induction hypothesis applied to $x$ and $v$
and the triangle inequality, we have
$$d_R(u,v)=\ell+d_R(x,v)\ge d_H(u,x)+\min(d_H(x,v),d_H(x,B))\ge \min(d_H(u,v),d_H(u,B)).$$

Let $X\subseteq V(G)\cap V(H)$ be the largest set of vertices such that
$d_H(u,v)>4d$ for all distinct $u\in X$ and $v\in X\cup B$.  By (\ref{eq:fabran}),
it follows that $d_G(u,v)\ge\frac{d_R(u,v)}{2}>2d$ for each $u\in X$ and $v\in X\cup \partial G$.
Consequently, $X$ is a $d$-isolated set in $G$ and $|X|\le \beta_d(G)$.
It remains to relate $|\partial G_f|$ to $|X|$.
If $\Sigma$ is the projective plane, then $H$ is a cycle,
$|X|\ge \max(1,\lfloor|V(H)|/(4d+2)\lfloor)\ge |V(H)|/(8d+4)$
and $|\partial G_f|=2|V(H)\cap V(G))|=|V(H)|\le (8d+4)|X|$,
implying the lemma holds.

Hence, we can assume $\Sigma$ is not the projective plane,
and thus $H$ contains a vertex of degree at least three.  Let $H'$ be the multigraph obtained
from $H$ by suppressing all vertices of degree two.  Since $H'$ has minimum degree at least three
and a drawing in $\Sigma$ with one $2$-cell face, Euler's formula (Theorem~\ref{thm:eulerfla}) gives
$$3|V(H')|\le 2|E(H')|=2(|V(H')|+k+g-1).$$
This implies $|V(H')|\le 2(k+g-1)$ and $|E(H')|\le 3(k+g-1)$, and thus the number $c$ of the components of $H-B$
is at most $3(k+g-1)$.  Each component $P$ is a path, and the maximality of $|X|$
implies $|V(P)\cap X|\ge |V(P)|/(4d+2)-2$.  Therefore,
\begin{align*}
|X|&\ge \frac{|V(H-B)|}{4d+2}-2c\ge \frac{|V(H-B)\cap V(G)|}{4d+2}-2c\\
&\ge \frac{|V(H)\cap V(G)|-|V(H')|-|\partial G|}{4d+2}-2c,
\end{align*}
and
$$|V(H)\cap V(G)|\le (4d+2)|X|+(24d+14)(k+g-1)+|\partial G|.$$
Consider a vertex $v\in V(H)\cap V(G)$, and let $b=1$ if $v\in\partial G$ and $b=0$ otherwise.
Note that when cutting along the net $(H,f)$, $v$ is split into $\deg_H(v)-b$ vertices. 
Consequently, using Euler's formula, we obtain
\begin{align*}
|\partial G_f|&=2|V(H)\cap V(G)|-\partial G+\sum_{v\in V(H)\cap V(G)} (\deg_H(v)-2)\\
&=2|V(H)\cap V(G)|-|\partial G|+2(|E(H)|-|V(H)|)\\
&\le 2|V(H)\cap V(G)|-|\partial G|+k+g-1\\
&\le (8d+4)|X|+(48d+29)(k+g-1)+|\partial G|.
\end{align*}
\end{proof}

In case $G$ comes from the hyperbolic family, we have $|V(G)|=O(|\partial G_f|)$, implying $\beta_d(G)=\Omega(|V(G)|/d)$.
The neighborhoods of vertices of the largest $d$-isolated set up to distance $d$ are disjoint, implying that most of them
have size $O(d)$.  In combination with the exponential growth guaranteed by Corollary~\ref{cor:expgrowth}, it follows
that these neighborhoods contain short non-contractible cycles.

\begin{corollary}\label{cor:hyperew}
For every $\gamma>1$, there exists a positive integer $d$ such that the following holds.
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma$.
If $G\in\GG$ is a graph with a boundary-perpendicular $2$-cell drawing in a surface of genus $g$ with $k$ holes,
then either
\begin{itemize}
\item $|V(G)|\le 2\gamma((48d+29)(k+g)+|\partial G|)$, or
\item $G$ contains a non-contractible cycle of length at most $2d+1$ disjoint from $\partial G$.
\end{itemize}
\end{corollary}
\begin{proof}
Choose $d$ as a minimum integer such that $(16d+8)\gamma\le \Bigl(\tfrac{\gamma}{\gamma-1}\Bigr)^d$.

By Lemma~\ref{lemma:isnet}, there exists a net $(H,f)$ perpendicular to $G$ such that
$|\partial G_f|\le (8d+4)\beta_d(G)+(48d+29)(k+g)+|\partial G|$.
Since $G\in\GG$, we have $|V(G)|\le |V(G_f)|<\gamma|\partial G_f|$.
If the first outcome of this corollary does not hold, then $\gamma((48d+29)(k+g)+|\partial G|)\le |V(G)|/2$.
Combining these inequalities, we obtain $|V(G)|<(16d+8)\gamma\beta_d(G)$.  Let $X$ be a $d$-isolated
set in $G$ of size more than $\tfrac{|V(G)|}{(16d+8)\gamma}$.  For each $v\in X$, let $G_v$ be the
subgraph of $G$ induced by vertices at distance at most $d$ from $v$.  Since $X$ is $d$-isolated,
these subgraphs are pairwise disjoint and disjoint from $\partial G$.  Consequently, there exists $v\in X$
such that $|V(G_v)|<(16d+8)\gamma\le \Bigl(\tfrac{\gamma}{\gamma-1}\Bigr)^d$.
By Corollary~\ref{cor:expgrowth}, this implies there exists a non-contractible closed walk in $G$ of length at most $2d+1$
containing $v$.  This walk is contained in $G_v$, and thus it is disjoint from $\partial G$.  Therefore,
there also exists a non-contractible cycle of length at most $2d+1$ disjoint from $\partial G$.
\end{proof}
In particular, in case the surface has no holes, the first outcome shows the graph has bounded size,
giving us the following consequence in combination with Corollary~\ref{cor:logew}.
\begin{corollary}
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma>1$.
If $G\in\GG$ is drawn in a surface of genus $g$ without boundary, then $G$ has edge-width $O(\log g)$.
\end{corollary}
By considering the class of graphs boundary-critical for $5$-list-coloring, this for example implies
that every locally planar graph (a graph drawn in a surface of genus $g$ with edge-width $\Omega(\log g)$)
is 5-list-colorable.

Corollary~\ref{cor:hyperew} has another important consequence, which in particular motivates the notion of strong hyperbolicity
we will introduce later.  To show this consequence, we need a standard bound on the number of disjoint homotopically non-equivalent
cycles that can be drawn in a fixed surface.
\begin{lemma}\label{lemma:disjnonc}
Let $\Sigma$ be a surface of genus $g$ with $k$ holes, where $k+g>1$, and let $C_1$, \ldots, $C_m$ be non-contractible cycles with disjoint drawings in $\Sigma$.
If $C_1$, \ldots, $C_m$ are pairwise homotopically non-equivalent, then $m\le 2k+3g-2$.
\end{lemma}
\begin{proof}
If $g=0$ and $k=1$, then all cycles in $\Sigma$ are contractible, and thus $m=0$.  Hence, we can assume $g\ge 1$ or $m\ge 2$.
Without loss of generality, we can assume $m$ is maximum, and thus every non-contractible cycle in $\Sigma$ disjoint
from $C_1$, \ldots, $C_m$ is homotopic to one of these cycles.  Moreover, for each hole, we can assume some cycle among $C_1$,
\ldots, $C_m$ traces its boundary: If there is one homotopic to the curve tracing the boundary, we can shift it to the boundary.
Otherwise, we can shift the other cycles away from the boundary and add a cycle homotopic to it.

Consequently, letting $H=C_1\cup\ldots\cup C_m$, every face $f$ of $H$ is homeomorphic to the interior of the sphere with at most three holes,
as otherwise we can draw a cycle non-homotopic to all cycles of $H$ in $f$.
If $f$ is an open disk, then since the cycle of $H$ bounding it is non-contractible, it must be one-sided and $H$
must consist of just this one cycle, i.e., $\Sigma$ is the projective plane, $g=1$, $k=0$, and $m=1$.  Consider a face of $f$ of
$H$ homeomorphic to an open cylinder.  If both cycles bounding $f$ are two-sided, then they are homotopic, and thus they must in
fact be the same cycle, i.e., $\Sigma$ is the torus, $g=2$, $k=1$, and $m=1$.  If both are one-sided, then
$\Sigma$ is the Klein bottle, $g=2$, $k=1$, and $m=2$.  Hence, if $f$ is an open cylinder, then exactly one of the cycles
bounding it is one-sided.  If $f$ is homeomorphic to the interior of the sphere with three holes, then all the cycles bounding it
must be $2$-sided, as otherwise we could add a $2$-sided cycle going around the boundary formed by an one-sided cycle.

Let $t$ denote the number of one-sided cycles among $C_1$, \ldots, $C_m$.  Then exactly $t$ of the faces of $H$
are homeomorphic to the open cylinder, implying $3(|F(G)|-t)+t=2(m-t)-k$, and thus
\begin{equation}\label{eq:djnc-nf}
|F(G)|=(2m-k)/3.
\end{equation}
Let $\Sigma'$ be the surface obtained from $\Sigma$ by patching all the holes, and let $H'$ be the multigraph obtained
from $H$ by suppressing all but one vertex of each cycle (so the cycles become loops) and inside each face of $H$ with $b\in\{2,3\}$
boundary cycles, drawing $b-1$ edges between them to turn the face into $2$-cell one.
Then $|V(H')|=m$, $|E(H')|=m+2|F(H)|-t$, $|F(H')|=|F(H)|+k$, and the drawing of $H'$ in $\Sigma'$ is $2$-cell.
By the Euler's formula (Theorem~\ref{thm:eulerfla}), we have
\begin{equation}\label{eq:djnc-ef}
g-2=|E(H')|-|V(H')|-|F(H')|=|F(H)|-t-k.
\end{equation}
Combining (\ref{eq:djnc-nf}) with (\ref{eq:djnc-ef}), we have
$m\le 2k+3(g+t)/2-3$.  Note that cutting along any of the cycles does not affect whether the remaining cycles are one-sided or not,
and that cutting along a one-sided cycle decreases the genus by one.  This implies $t\le g$,
and thus $m\le 2k+3g-3$.
\end{proof}

Consider a graph $G$ with a $2$-cell boundary-perpendicular drawing in a surface $\Sigma$, and let $H$
be another graph drawn in $\Sigma$.  For a positive integer $r$, we say $H$ is an \emph{$r$-circular skeleton} of $G$
if $V(H)\subseteq V(G)$, edges of $H$ intersect $G$ only in their ends, the boundary of each hole in $\Sigma$ is bounded
by a cycle forming a component of $H$, and all other components of $H$ are non-contactible cycles of length at most $r$.
We say $H$ is \emph{cycle-maximal} if $G$ does not contain a non-contractible cycle $K$ of length at most $r$
disjoint from $V(H)$ (note a cycle obtained by slightly shifting the edges of such a cycle $K$ could be added to $H$,
obtaining a larger $r$-circular skeleton).
A face $f$ of $H$ is \emph{cylindrical} if it is homeomorphic to the open cylinder and both cycles in $H$ that bound it
are disjoint from the boundary of $\Sigma$ (and thus they have length at most $r$).

\begin{lemma}\label{lemma:hyperstruct}
Let $\gamma>1$ be a real number and let $d$ be the corresponding integer from Corollary~\ref{cor:hyperew}.
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma$.
Let $G\in\GG$ be a graph with a boundary-perpendicular $2$-cell drawing in a surface of genus $g$ with $k$ holes
and let $H$ be a cycle-maximal $(2d+1)$-circular skeleton of $G$.  For each face $f$ of $H$,
\begin{itemize}
\item the surface $\Sigma_f$ in which $G_f$ is drawn has $k_f\le 5k+6g$ holes and genus $g_f\le g$,
\item $G_f$ does not contain any non-contractible cycle of length at most $2d+1$ disjoint from $\partial G_f$, and
\item $|\partial G_f\setminus\partial G|\le 2(2d+1)k_f$.
\end{itemize}
Moreover, let $H'$ be the minimal subgraph of $H$ such that
$H'$ is a union of components of $H$ and the non-cylindrical faces of $H$ and $H'$ coincide.
Then $H'$ has at most $2k+3g-2$ cylindrical and at most $2k+3g-1$ non-cylindrical faces.
\end{lemma}
\begin{proof}
Let $\HH$ be the set cycles of $H$ disjoint from the boundary of $\Sigma$, and let $\HH_1$, \ldots, $\HH_m$
be the partition of $\HH$ into classes of homotopically equivalent cycles, where $m\le 2k+3g-2$ by Lemma~\ref{lemma:disjnonc}.
For $i=1,\ldots, m$, if $|\HH_i|=1$, then let $\Sigma_i$ be the subset of $\Sigma$ consisting of the drawing of the cycle in $\HH_i$,
otherwise let $\Sigma_i\subset \Sigma$ be the maximal subcylinder of $\Sigma$ bounded by cycles in $\HH_i$.
Observe that $H\cap \Sigma_i$ contains exactly the cycles in $\HH_i$, all faces of $H$ contained in $\Sigma_i$
are cylindrical, and for every cylindical face $f$, there exists $i$ such that $f\subset \Sigma_i$.
Hence, the number of non-cylindrical faces is equal to the number of components of $\Sigma\setminus\bigcup_{i=1}^m \Sigma_i$,
which is at most $m+1$ (it can be smaller if for some $i$, $\HH_i$ consists of non-separating cycles).
Note that $H'$ consists exactly of the cycles of $\HH_i$ forming the boundary of $\Sigma_i$, for $i=1,\ldots,m$,
and thus $H'$ has at most $m$ cylindrical faces.

Consider a face $f$ of $H$.  By the cycle-maximality of $H$, every non-contractible cycle in $G_f$ disjoint from $\partial G_f$
has length greater than $2d+1$.  Let $k_f$ denote the number of holes of the surface $\Sigma_f$ in which $G_f$ is drawn, and $g_f$ its
genus; clearly $k_f\le k+2m<5k+6g$ and $g_f\le g$.  Since $H$ is cycle-maximal, $G_f$ cannot contain a non-contractible cycle
of length at most $2d+1$ disjoint from $\partial G_f$.  Moreover,
$|\partial G_f\setminus \partial G|\le 2(2d+1)k_f$; the factor of two accounts for the possibility that the
boundary walk corresponds to a one-sided cycle.
\end{proof}
Note that since $\GG$ is cutting-closed, we have $G_f\in\GG$, and thus Corollary~\ref{cor:hyperew}
implies
\begin{align*}
$|V(G_f)|&\le 2\gamma((48d+29)(k_f+g_f)+|\partial G_f|)\\
&\le 2\gamma|\partial G\cap \partial G_f|+2\gamma(52d+31)(k_f+g_f)\\
&\le 2\gamma|\partial G\cap \partial G_f|+14\gamma(52d+31)(k+g),
\end{align*}
and thus all the pieces of the decomposition given by Lemma~\ref{lemma:hyperstruct} have bounded size.
Subject to rather weak additional assumptions, this decomposition can be used to test presence of a subgraph
belonging to the fixed hyperbolic family.  This is of course important
for our intended coloring applications, as this corresponds to testing whether the given graph is colorable by detecting
the presence of one of the obstructions.

\subsection{Finding a subgraph belonging to a hyperbolic class}

In order to apply Lemma~\ref{lemma:hyperstruct} algorithmically, we of course need to be able to efficiently
find a cycle-maximal $r$-circular skeleton.  As we do not want to focus on this (albeit important) technicality, let
us just use the following result without a proof.

\begin{theorem}[Cabello and Mohar~\cite{cabello-sepcurv}]\label{thm:findnc}
For every surface $\Sigma$ and integer $r$, there exists an algorithm that given a graph $G$ with boundary-perpendicular $2$-cell drawing
in $\Sigma$ and a set $X\subseteq V(G)$, in time $O(|V(G)|)$ either returns a non-contractible cycle in $G$ of length at most $r$ disjoint
from $X$, or correctly decides no such cycle exists.
\end{theorem}

To find a cycle-maximal $r$-circular skeleton, we start with $X=\partial G$ and repeatedly find non-contractible cycles and add them to $X$.
\begin{corollary}\label{cor:findcsc}
For every surface $\Sigma$ and integer $r$, there exists an algorithm that given a graph $G$ with boundary-perpendicular $2$-cell drawing
in $\Sigma$, in time $O(|V(G)|^2)$ returns a cycle-maximal $r$-circular skeleton of $G$.
\end{corollary}
Let us remark that by more involved algorithmic techniques, the time complexity can be improved to $O(|V(G)|)$, using e.g. ideas presented
in~\cite{trfree6}.

Secondly, it is not very hard to construct hyperbolic families for which the membership is algorithmically undecidable,
by encoding all positive instances of the halting problem into long cylindrical pieces.  Thus, we need additional property
that ensures algorithmic tractability of the class.  Fortunately, a fairly weak one suffices.

Let $H_1$ and $H_2$ be graphs with boundary-perpendicular drawing in the cylinder $\Pi$ such that $\partial H_1=\partial H_2$.
Consider two graphs $G_1$ and $G_2$ with boundary-perpendicular drawings in the same surface $\Sigma$.
We say that $G_2$ is \emph{obtained from $G_1$ by replacing $H_1$ by $H_2$} if 
there exists a cylinder $\Pi'\subseteq \Sigma$ whose boundary intersects the drawings of $G_1$ and $G_2$ only in vertices
and a homeomorphism $\theta:\Pi\to\Pi'$ such that
\begin{itemize}
\item the subgraphs of $G_1$ and $G_2$ obtained by deleting vertices and edges drawn in the interior of $\Pi'$ are identical, and
\item $\theta(H_1)=G_1\cap \Pi'$ and $\theta(H_2)=G_2\cap\Pi'$.
\end{itemize}
For a class of graphs $\GG$ drawn on surfaces, we write $H_1\equiv_{\GG} H_2$ if for every graph $G_1$ and a graph $G_2$ obtained
from $G_1$ by replacing $H_1$ by $H_2$, we have $G_1\in \GG$ if and only if $G_2\in\GG$.

To clarify this definition, let us give an example.  Recall that for a graph $G$ and its subgraph $F$,
$\EE_{F,k}(G)$ denotes the set of $k$-colorings of $F$ that extend to a $k$-coloring of $G$.
Suppose $\GG$ is the class of graphs with boundary-perpendicular drawings in surfaces
that are boundary-critical for 5-coloring.  For a graph $H\in \GG$ drawn in the cylinder, let 
let $\Gamma_H=(\EE_{\partial H,5}(H),I_H,T_H)$, where $I_H$ is the
set of isolated vertices of $H$ (necessarily contained in $\partial H$, since $H$ is boundary-critical for $5$-coloring),
and $T_H=\{\EE_{\partial H,5}(H-e):e\in E(H)\}$.  We claim that for graphs $H_1,H_2\in\GG$ are drawn in the cylinder
with $\partial H_1=\partial H_2$, if $\Gamma_{H_1}=\Gamma_{H_2}$, then $H_1\equiv_{\GG} H_2$.
Indeed, let $G_2$ be obtained from a graph $G_1$ drawn in a surface by replacing $H_1$ by $H_2$ and let $\theta$ be the corresponding
homeomorphism.  By symmetry, it is enough to show that if $G_1\in \GG$, then $G_2\in\GG$.
Note that all isolated vertices of $G_2$ are contained in $\partial G_2=\partial G_1$,
since all isolated vertices of $G_1$ are contained in $\partial G_1$ by its boundary-criticality and since $I_{H_2}=I_{H_1}$.
Hence, to establish that $G_2$ is boundary-critical for $5$-coloring, it suffices to show that for every edge $e\in E(G_2)$,
there exists a $5$-coloring of $\partial G_2$ extending to a $5$-coloring of $G_2-e$, but not to a $5$-coloring of $G_2$.
If $e\in E(G_2)\setminus \theta(E(H_2))$, then let $e'=e$.  If $e\in \theta(E(H_2))$, then since $T_{H_1}=T_{H_2}$, there exists an edge
$e_1\in E(H_1)$ such that $\EE_{\partial H_1,5}(H_1-e_1)=\EE_{\partial H_2,5}(H_2-\theta^{-1}(e))$; in this case, we let $e'=\theta(e_1)$.
Since $G_1$ is boundary-critical for $5$-coloring, there exists a $5$-coloring $\psi$ of $\partial G_1$ extending to $5$-coloring
of $G_1-e'$, but not to a $5$-coloring of $G_1$.  Since  $\EE_{\partial H_1,5}(H_1-\theta^{-1}(e'))=\EE_{\partial H_2,5}(H_2-\theta^{-1}(e))$
and $\EE_{\partial H_1,5}(H_1)=\EE_{\partial H_2,5}(H_2)$, it follows that $\psi$ extends to a $5$-coloring of $G_2-e$, but
not to a $5$-coloring of $G_2$, as required.

Clearly, $\equiv_{\GG}$ is an equivalence relation, and we have the following composition result.
\begin{observation}\label{obs:compequiv}
Let $\GG$ be a class of graphs drawn on surfaces.
Let $G$ and $G'$ be graphs with boundary-perpendicular drawings in the cylinder $\Sigma$.
Let $H$ be a graph with drawing in $\Sigma$ perpendicular to both $G$ and $G'$ such that $H$ consists of three disjoint non-contractible cycles,
two of which trace the boundaries of the holes of $\Sigma$, and assume that $G\cap H=G'\cap H$.  Let $f_1$ and $f_2$ be the faces of $H$.
If $G_{f_1}\equiv_{\GG} G'_{f_1}$ and $G_{f_2}\equiv_{\GG} G'_{f_2}$, then $G\equiv_{\GG} G'$.
\end{observation}
\begin{proof}
Replacing $G$ by $G'$ amounts to replacing $G_{f_1}$ by $G'_{f_1}$ and $G_{f_2}$ by $G'_{f_2}$, and both of these operations
preserve the membership in $\GG$.
\end{proof}

We say that the class $\GG$ has \emph{finite index} if
for every finite set $X$ of vertices drawn in the boundary of cylinder, $\equiv_{\GG}$ has only finitely many
classes of equivalence among the graphs $H$ with boundary-perpendicular drawing in the cylinder such that $\partial H=X$.
Continuing the example of graphs boundary-critical for 5-coloring, there are only finitely many choices for $\Gamma_H$:
there are only $2^{5^{|X|}}$ choices for a set $\EE_{\partial H,5}(H)$ of colorings, only $2^{2^{5^{|X|}}}$ choices for a set $T_H$ of
sets of colorings, and at most $2^{|X|}$ choices for the set $I_H$ of boundary vertices.
This shows that $\equiv_{\GG}$ has at most $2^{|X|+5^{|X|}+2^{5^{|X|}}}$ classes of equivalence for each set $X$,
showing that $\GG$ has finite index.
Analogous argument can be employed to show that the classes of boundary-critical graphs have finite index
for pretty much all natural variants of graph coloring using a bounded number of colors.
In particular, this is always the case if the membership in the class $\GG$ depends only on the abstract graph $G$
and the subset $\partial G$ of its vertices (rather than properties depending on the embedding) and can be expressed
in the monadic second-order logic~\cite{courcelle}.

In addition to finite index, we add another technicality---that the class is closed under homeomorphisms.
This of course is the case for all classes relevant to us, and simplifies the formulations a bit.  For example,
it allows us to state the following definition without having to consider self-homeomorphisms of the surface.
Consider graphs $G$ and $H$ with boundary-perpendicular drawing in a surface $\Sigma$.  We say $H$ \emph{appears} in $G$
if $\partial H\subseteq \partial G$ and the drawing of $H$ is a subdrawing of the drawing of $G$.
Note that it is possible for several different graphs from $\GG$ to appear in $G$
(even ones with the same boundary), when they happen to have a common supergraph.  This motivates the following definitions.
For a graph $H$ with boundary-perpendicular drawing in the cylinder, let $[H]_\GG$ denote the equivalence class of $H$
under $\equiv_{\GG}$.  From a more practical perspective, in an algorithm, we would assign distinct identifiers (e.g.,
consecutive integers) to the equivalence classes and $[H]_\GG$ would then refer to this identifier.
For a graph $G$ with boundary-perpendicular drawing in the cylinder, let $\type_\GG(G)=\{[H]_\GG:\text{$H$ appears in $G$}\}$.
Naturally, types compose as follows.
\begin{observation}\label{obs:comptype}
Let $\GG$ be a homeomorphism-closed class of graphs drawn on surfaces.
Let $G$ and $G'$ be graphs with boundary-perpendicular drawings in the cylinder $\Sigma$.
Let $H$ be a graph with drawing in $\Sigma$ perpendicular to both $G$ and $G'$ such that $H$ consists of three disjoint non-contractible cycles,
two of which trace the boundaries of the holes of $\Sigma$, and assume that $G\cap H=G'\cap H$.  Let $f_1$ and $f_2$ be the faces of $H$.
If $\type_{\GG}(G_{f_1})=\type_{\GG}(G'_{f_1})$ and $\type_{\GG}(G_{f_2})=\type_{\GG}(G'_{f_2})$, then $\type_{\GG}(G)=\type_{\GG}(G')$.
\end{observation}
\begin{proof}
Suppose that a graph $F\in\GG$ appears in $G$; then $F_{f_1}$ appears in $G_{f_1}$ and $F_{f_2}$ appears in $G_{f_2}$.
Since $\type_{\GG}(G_{f_1})=\type_{\GG}(G'_{f_1})$ and $\type_{\GG}(G_{f_2})=\type_{\GG}(G'_{f_2})$,
for $i\in \{1,2\}$, there exists a graph $F'_i$ appearing in $G'_{f_i}$ such that $F'_i\equiv_\GG F_{f_i}$.
Let $F'$ be the subgraph of $G'$ such that $F'_{f_1}=F'_1$ and $F'_{f_2}=F'_2$; by Observation~\ref{obs:compequiv},
$F'\equiv_\GG F$.  Hence, for every graph appearing in $G$, an equivalent graph appears in $G'$, and thus $\type_{\GG}(G)\subseteq \type_{\GG}(G')$.
Hence, by symmetry between $G$ and $G'$ we have $\type_{\GG}(G)=\type_{\GG}(G')$.
\end{proof}

Note that if $\GG$ has finite index, then for each finite set $X$ of points in the boundary of the cylinder, there
are only finitely many possible values for $\type_\GG(G)$ for graphs $G$ with $\partial G=X$.
This gives us the following algorithm.
\begin{lemma}\label{lemma:comptype}
Let $\GG$ be a homeomorphism-closed class of graphs drawn on surfaces of finite index and let $r$ be a positive integer.
Then there exists an integer $b_0$ and a linear-time algorithm that is given as an input
\begin{itemize}
\item a graph $G$ with a boundary-perpendicular drawing in the cylinder,
\item a graph $H$ with drawing perpendicular to $G$, such that each component of $H$ is a non-contractible cycle of length at most $r$
and two of these cycles bound the holes of the cylinder, and
\item for each face $f$ of $H$, the set $\type_\GG(G_f)$, and
\end{itemize}
returns a graph $G'$ with a boundary-perpendicular drawing in the cylinder such that $\partial G'=\partial G$,
$\type_{\GG}(G')=\type_{\GG}(G)$, and $|V(G')|\le b_0$.
\end{lemma}
\begin{proof}
For all integers $0\le a_1,a_2\le r$, let us fix some sets $X_{a_1,a_2}$ in the boundary of the cylinder
containg $a_1$ points in one of the components of the boundary and $a_2$ points in the other one.
Let $\CC_{a_1,a_2}$ denote the set of all classes of equivalence of $\equiv_{\GG}$ on the graphs $H\in \GG$
with boundary-perpendicular drawing in the cylinder such that $\partial H\subseteq X_{a_1,a_2}$.  For each $T\subseteq \CC_{a_1,a_2}$,
if there exist a graph $Z$ with boundary-perpendicular drawing in the cylinder such that $\type_{\GG}(Z)=T$, let us
fix one such graph arbitrarily and let it be denoted by $Z_T$; this choice will be hardwired in our algorithm.
Note that since $\GG$ has finite index, we perform only finitely many choices.
Let $b_0$ be the maximum of $|V(Z_T)|$ over all choices of $a_1$, $a_2$, and $T$.

Let $a_1$ and $a_2$ denote the numbers of intersections of $\partial G$ with the components of the boundary of the cylinder.
The algorithm only needs to determine $\type_{\GG}(G)$, and then it can output (the appropriate homeomorphic image of) $Z_{\type_{\GG}(G)}$.
Let $C_0$, $C_1$, \ldots, $C_m$ be the components of $H$ in order along the cylinder, and for $i\in\{1,\ldots, m\}$, let $f_i$
denote the face of $H$ bounded by $C_{i-1}$ and $C_i$, and $f'$ the face of $C_0\cup C_i$ bounded by $C_0$ and $C_i$.
By Observation~\ref{obs:comptype}, $\type_{\GG}(G_{f'_{i-1}})$ and $\type_{\GG}(G_{f_i})$ uniquely determine
$\type_{\GG}(G_{f'_i})$, and since there are only finitely many possible values for types, the function $\gamma$ describing
this transition can be hardwired in the algorithm.  Hence, letting $T_1=\type_{\GG}(G_{f_1})$ and for $i=2,\ldots,m$,
$T_i=\gamma(T_{i-1}, \type_{\GG}(G_{f_i}))$, we in linear time determine $T_m=\type_{\GG}(G_{f'_m})=\type_{\GG}(G)$.
\end{proof}

Lemmas~\ref{lemma:hyperstruct} and \ref{lemma:comptype} now naturally combine to give the following algorithm.

\begin{theorem}\label{thm:kernelhyp}
Let $\GG$ be a cutting- and homeomorphism-closed class of graphs hyperbolic with Cheeger constant $\gamma>1$ and of finite index.
For every surface $\Sigma$, there exists a constant $b$ and a polynomial-time algorithm that 
for any graph $G$ with a boundary-perpendicular drawing in $\Sigma$ returns a graph $G'$ with a boundary-perpendicular drawing
in $\Sigma$ such that $\partial G'=\partial G$, $|V(G')|\le |V(G)|+b$, and some graph from $\GG$ appears in $G$
if and only if some graph with from $\GG$ with at most $2\gamma |\partial G| + b$ vertices appears in $G'$.
\end{theorem}
\begin{proof}
If $\Sigma$ is the sphere, we can return an empty graph, since no graph from $\GG$ is drawn in the sphere.
Hence, we can assume that $k+g>0$,
Let $d$ be the constant from Corollary~\ref{cor:hyperew} for the given $\gamma$.
Let $g$ be the genus and $k$ the number of holes of $\Sigma$.
Let $r$ and $c$ be the constants from Lemma~\ref{lemma:hyperstruct}, and let $b_0$ be the constant corresponding to $\GG$ and $r=2d+1$
in Lemma~\ref{lemma:comptype}.  Let $c=2\gamma(52d+31)(k+g)$ and let $b=(4k+6g)\max(7c(k+g), b_0)$.

Let us now describe the algorithm.  First, we apply the algorithm from Corollary~\ref{cor:findcsc} to find
a maximar $(2d+1)$-circular skeleton $H$ of $G$, and find its minimal subgraph $H'$ such that
$H'$ is a union of components of $H$ and the non-cylindrical faces of $H$ and $H'$ coincide.
By Lemma~\ref{lemma:hyperstruct}, the number of faces of $H'$ is at most $4k+6g$.

Consider any face $f$ of $H$, and a graph $F\in\GG$ appearing in $G_f$.
Since $G_f$ does not contain a non-contractible cycle of length at most $2d+1$ disjoint from $\partial G_f$,
neither does $F$.  Moreover, $|\partial F\setminus\partial G|\le |\partial G_f\setminus\partial G|\le 2(2d+1)k_f$,
where $k_f\le 5k+6g$ is the number of holes and $g_f\le g$ is the genus of the surface $\Sigma_f$ in which $G_f$ is drawn.
Hence, Corollary~\ref{cor:hyperew} implies 
$|V(F)|\le 2\gamma|\partial G\cap \partial F|+c(k_f+g_f)\le 2\gamma|\partial G\cap \partial F|+7c(k+g)$.

Suppose now $f$ is a cylindrical face. Then $\partial G\cap \partial F=\emptyset$, $k_f=2$, and $g_f=0$,
and thus $|V(F)\le 2c$.
Consequently, we can determine $\type_{\GG}(G_f)$ by testing the presence of finitely many such graphs $F$, which can be done in polynomial
time (the set of graphs in $\GG$ with at most $2c$ vertices and their equivalence classes can be hardwired in the algorithm).
Applying Lemma~\ref{lemma:comptype} for each cylindrical face $h$ of $H'$, we can find a graph $Z^h$
with a boundary-perpendicular drawing in the cylinder such that $\partial Z^h=\partial G_h$, $\type_{\GG}(Z^h)=\type_{\GG}(G_h)$, and $|V(Z^h)|\le b_0$.
Let $G'$ be obtained for $G$ by, for each cylindrical face $h$ of $H'$, replacing $G_h$ by $Z^h$.
Clearly $\partial G'=\partial G$, and since $H'$ has at most $4k+6g$ faces, we have $|V(G')|\le |V(G)|+b$.

Suppose now that some graph $W\in\GG$ appears in $G$.  For each cylindrical face $h$ of $H'$,
since $\type_{\GG}(Z^h)=\type_{\GG}(G_h)$ and $W_h\subseteq G_h$, there exists a subgraph $F^h\subseteq Z^h$ such that
$\partial F^h=\partial W_h$ and $F^h\equiv_{\GG} W_h$.  Let $W'$ be obtained from $W$ by, for each cylindrical face $h$ of $H'$,
replacing $W_h$ by $F^h$.  By the definition of $\equiv_{\GG}$, we have $W'\in\GG$, and clearly $W'$ appears in $G'$.
For each cylindrical face $h$ of $H'$,
we have $|V(W'_h)|=|V(F^h)|\le |V(Z^h)|\le b_0$, while for each non-cylindrical face $h$ of $H'$
we have $|V(W'_h)|=|V(W_h)|\le 2\gamma|\partial W_h\cap \partial G|+7c(k+g)$.
Summing these inequalities, we obtain $|V(W')|\le 2\gamma |\partial G| + b$.  

Conversely, it is easy to see that any graph appearing in $G'$ can be transformed to a graph appearing of $G$.
\end{proof}

In particular, we obtain the following algoritm for graphs without a boundary.
\begin{corollary}\label{cor:testhyp}
Let $\GG$ be a cutting- and homeomorphism-closed class of graphs hyperbolic with Cheeger constant $\gamma>1$ and of finite index.
For every surface $\Sigma$ without a boundary, there exists a polynomial-time algorithm that,
given a graph $G$ drawn in $\Sigma$, determines whether some graph from $\GG$ appears in $G$.
\end{corollary}
\begin{proof}
Let $G'$ be the graph obtained by applying the algorithm from Theorem~\ref{thm:kernelhyp}. We only need to test whether some graph from $\GG$
with at most $b$ vertices appears in $G'$, and the finite list of such graphs can be hardwired in the algorithm.
\end{proof}

Let us quickly comment on the time complexity of the algorithms.  Naively, to test the presence of a subgraph with a fixed number
$b$ vertices in a graph with $n$ vertices, we would need time $O(n^b)$ to test all possibilities.  This is polynomial, but clearly
not very satisfying.  However, it turns out that much more efficient algorithms exist for graphs drawn on surfaces.  Using
e.g. a variation of the algorithm of Eppstein~{bib-eppstein99}, we can implement the subgraph tests in Theorem~\ref{thm:kernelhyp}
and Corollary~\ref{cor:testhyp} in linear time, with only the multiplicative constant depending on $b$.

For example, taking $\GG$ be the class of graphs boundary-critical for $5$-list-coloring, Corollary~\ref{cor:testhyp}
gives us a polynomial-time algorithm that for graphs in any fixed surface determines whether they are $5$-list-colorable.
% TODO -- discuss finding coloring, precoloring extension
