...
then either
\begin{itemize}
\item $|V(G)|\le 2\gamma(4g+2k+1)|\partial G|+2(13g+7k)\gamma^2$, or
\item $G$ contains a non-contractible cycle of length less than $4(2g+k)\gamma$ disjoint from $\partial G$.
\end{itemize}
\end{corollary}
\begin{proof}
If the second outcome does not hold, by Lemma~\ref{lemma:linnet}, there exists a net $(H,f)$ perpendicular to $G$ such that
$$|\partial G_f|<\frac{|V(G)|}{2\gamma}+(4g+2k+1)|\partial G|+(13g+7k)\gamma.$$
Since $\GG$ is cutting-closed and hyperbolic with Cheeger constant $\gamma$,
we have $|V(G)|\le |V(G_f)|<\gamma|\partial G_f|$, and thus
$$\frac{|V(G)|}{2\gamma}\le (4g+2k+1)|\partial G|+(13g+7k)\gamma,$$
implying the first outcome.
\end{proof}
In particular, in case the surface has no holes, the first outcome shows the graph has bounded size, and thus
it contains a non-contractible cycle of bounded length.
\begin{corollary}
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma>1$.
If $G\in\GG$ is drawn in a surface of genus $g$ without boundary, then $G$ has edge-width at most $26\gamma^2g$.
\end{corollary}
