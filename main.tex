...
\begin{theorem}[Heawood's formula, list coloring setting]
...
As we will see in the next chapter (Corollary~\ref{cor:small-medium}), a graph drawn in a surface of genus 2 is 6-colorable
...
\begin{theorem}[Dirac~\cite{dirac1957short},  Albertson and Hutchinson~\cite{albertson1979three}]\label{thm:dirac}
Suppose $G$ is a a graph drawn in a surface of genus $g\neq 0$.  If $\omega(G)<H(g)$, then $G$ is $(H(g)-1)$-colorable.
\end{theorem}

... % AMHERE  ??? Include proof in the critical graphs chapter?
...
For graphs with a small number of vertices, the following bound by Dirac~\cite{diracdens} beats the one from Theorem~\ref{thm:critdens}
...
\begin{theorem}
Suppose $G$ is a a graph drawn in a surface of genus $g\ge 4$.  If $\omega(G)<H(g)$, then $G$ is $(H(g)-1)$-colorable.
\end{theorem}
\begin{proof}
...
k^2-2k-(6g+10)&\le 0\\
\frac{3+\sqrt{24g+1}}{2}<H(g)-1=k&\le \Bigl\lfloor\frac{2+\sqrt{24g+44}}{2}\Bigr\rfloor.
\end{align*}
Note that $\sqrt{24g+44}-\sqrt{24g+1}-1$ is decreasing in $g$ and negative for $g=19$, hence we have $g\le 18$.
Furthermore, numerical calculations show that
\begin{itemize}
\item for $15\le g\le 18$, $H(g)-1=12$ while the right-hand side is $11$,
\item for $12\le g\le 14$, $H(g)-1=11$ while the right-hand side is $10$,
\item for $g\in\{10,11\}$, $H(g)-1=10$ while the right-hand side is $9$,
% ?? case g=9 currently fails, fix this somehow!
...
\end{proof}

% add remarks on other genera, choosability
...
this case is beyond the scope of this book, but we will discuss at least some of the ideas in the following
section).
...
As we have seen, for $k\ge 4$, the set $\FF_{k,4,g}$ is finite. ...
...
