...
For any finite abelian group $\mathbf{A}$,
the number of proper $|\mathbf{A}|$-colorings of a connected plane multigraph $G$ is equal to $|\mathbf{A}|$ times the number
of nowhere-zero $\mathbf{A}$-flows in the dual $D$ of $G$.
\end{corollary}
...
of its dual $H^\star$ by elements of $\mathbf{Z}_2^2$ as described at the beginning of this section.
\end{proof}
...
of a plane connected multigraph $G$, and let $f$ be the derived nowhere-zero $\mathbf{Z}$-flow in the dual of $G$.
Then, all values of $f$ are $\pm 1$, \ldots, $\pm (k-1)$.

Motivated by this observation, let us say that a $\mathbf{Z}$-flow is a \emph{$k$-flow} if all its values are at most $k-1$ in absolute value.
If $f$ is a (nowhere) $k$-flow, we can interpret its values as elements of $\mathbf{Z}_k$, and thus $f$ can also be viewed as a (nowhere-zero)
$\mathbf{Z}_k$-flow.  Together with Lemma~\ref{lemma:flowtocolor} and the argument from the previous paragraph,
...
\end{observation}
As a consequence, a planar graph has a nowhere-zero $k$-flow if and only if it has a nowhere-zero $\mathbf{Z}_k$-flow.
A slightly more involved argument shows that this is true in general graphs as well.

\begin{lemma}
Let $G$ be a multigraph and let $k$ be a positive integer.  For every $\mathbf{Z}_k$-flow $f_0$ in $G$, there exists
a $k$-flow $f_1$ in $G$ such that $\bar{f_1}(uv)\bmod k=\bar{f_0}(uv)$.  In particular, if $f_0$ is nowhere-zero,
then $f_1$ is nowhere-zero as well.
\end{lemma}
\begin{proof}
Let $\vec{G}$ be an arbitrary orientation of $G$, and for each directed edge $uv\in E(\vec{G})$, let
$f'_1(uv)=\bar{f}_0(uv)$, where the values of $f_0$ are interpreted as integers from $\{0,1,\ldots, k-1\}$.
For $v\in V(G)$, let us define
$$r'(v)=\sum_{vw\in E(\vec{G})} f'_1(vw)-\sum_{uv\in E(\vec{G})} f'_1(uv),$$
denoting the excess of the amount leaving $v$ over the one entering $v$.
Note that $r'(v)$ is divisible by $k$, since $f_0$ is a $\mathbf{Z}_k$-flow.
Let $r(v)=r'(v)/k$.

Let $\vec{H}$ be the spanning subgraph of $\vec{G}$ with $E(\vec{H})=\{e\in E(\vec{G}): f_0(e)\neq 0\}$.
Consider any set $S\subseteq V(\vec{H})$, and let $I(S)$ and $O(S)$ be the sets of edges of $\vec{H}$ entering
and leaving $S$, respectively.  We have
\begin{align*}
\sum_{v\in S} r(S)&=\frac{1}{k}\sum_{v\in S}\left(\sum_{vw\in E(\vec{G})} f'_1(vw)-\sum_{uv\in E(\vec{G})} f'_1(uv)\right)
&\frac{1}{k}\left(\sum_{e\in O(S)} f'_1(e)-\sum_{e\in I(S)} f'_1(e)\right),
\end{align*}
...
-|I(S)|\le \sum_{v\in S} r(S)\le |O(S)|.
\end{equation}
Let us give each edge of $\vec{H}$ capacity $1$ and each vertex $v$ of $\vec{H}$ demand $r(v)$.
Since (\ref{eq:bigcut}) holds for all $V(\vec{H})$, we conclude from the max-flow min-cut theorem
that there exists $f''_1:E(\vec{H})\to\{0,1\}$ such that
$$\sum_{vw\in E(\vec{H})} f''_1(vw)-\sum_{uv\in E(\vec{H})} f''_1(uv)=r(v)$$
for all $v\in V(\vec{H})$.  Let us extend $f''_1$ to $\vec{G}$ by setting $f''_1(e)=0$ for all $e\in E(\vec{G})\setminus E(\vec{H})$.

Now, we can define $f_1(e)=f'_1(e)-kf''_1(e)$ for all $e\in E(\vec{G})$.  By the choice of $f''_1$, we conclude that $f_1$ is a $\mathbf{Z}$-flow
...
If $\mathbf{A}'$ and $\mathbf{A}$ are two finite abelian groups, $|\mathbf{A}'|\le |\mathbf{A}|$, and
a multigraph $G$ has a nowhere-zero $\mathbf{A}'$-flow, then $G$ also has a nowhere-zero $\mathbf{A}$-flow.
\end{corollary}

\section{Basic results on existence of nowhere-zero flows}\label{sec:basicflow}

It is easy to decide whether a graph has a nowhere-zero $\mathbf{Z}_2$-flow, based on the following observation.
\begin{observation}\label{obs:euler}
If $G$ is an Eulerian graph, then $G$ has a nowhere-zero $\mathbf{A}$-flow for any non-trivial abelian group $\mathbf{A}$.
Conversely, if $G$ has a nowhere-zero $\mathbf{Z}_2$-flow, then $G$ is Eulerian.
\end{observation}
\begin{proof}
If $G$ is Eulerian, then each component of $G$ contains a closed walk that passes through each edge exactly once.
Orienting the edges of $G$ along these walks and assigning to each of them the same non-zero element of $\mathbf{A}$
gives a nowhere-zero $\mathbf{A}$-flow in $G$.

A nowhere-zero $\mathbf{Z}_2$-flow necessarily assigns value $1$ to each edge of $G$, the flow conservation law
thus implies that all vertices of $G$ have even degree, and thus $G$ is Eulerian.
\end{proof}

In general, it is hard to decide whether a given graph has a nowhere-zero $\mathbf{A}$-flow for a prescribed
finite abelian group $\mathbf{A}\neq \mathbf{Z}_2$.  However, there are a number of results that are helpful in particular cases.
Let us start with a basic observation about edge-cuts.  If $f$ is a flow in a multigraph $G$ and $S\subseteq V(G)$,
let us define $$\bar{f}(S)=\sum_{e=uv\in E(G),u\in S,v\not\in S} \bar{f}(e).$$

\begin{observation}\label{obs:flowoncut}
Let $\mathbf{A}$ be an abelian group.  If $f$ is an $\mathbf{A}$-flow in a multigraph $G$, then $\bar{f}(S)=0$ for all $S\subseteq V(G)$.
\end{observation}
\begin{proof}
Note that
$$\bar{f}(S)=\sum_{u\in S}\sum_{e=uv\in E(G)} \bar{f}(e)=0,$$
where the first equality holds since if an edge $uv$ has both ends in $S$, then the sum contains
terms $\bar{f}(uv)$ and $\bar{f}(vu)=-\bar{f}(uv)$ which cancel each other, leaving only the terms
from the sum in the definition of $\bar{f}(S)$.
\end{proof}

\begin{corollary}\label{cor:smallcutflow}
Let $\mathbf{A}$ be an abelian group and let $G$ be a multigraph.  If $G$ has a nowhere-zero $\mathbf{A}$-flow $f$,
then $G$ is bridgeless.  Furthermore, if $C=\{e_1,e_2\}$ is a $2$-edge-cut in $G$, where $e_1=u_1v_1$ and $e_2=u_2v_2$
labelled so that $u_1$ and $u_2$ are in the same component of $G-C$, then $\bar{f}(e_1)=-\bar{f}(e_2)$.
\end{corollary}
\begin{proof}
If $G$ had a bridge $e$, then there exists a set $S\subset V(G)$ such that $e$ is the only edge with one end in $S$ and
the other end outside of $S$, and $\bar{f}(e)=\bar{f}(S)=0$ by Observation\ref{obs:flowoncut}, contradicting the assumption
that $f$ is nowhere-zero.

In the case of a $2$-edge-cut, let $S$ be the vertex set of the component of $G-C$ containing $u_1$ and $u_2$.
We have $\bar{f}(e_1)+\bar{f}(e_2)=\bar{f}(S)=0$ by Observation\ref{obs:flowoncut}, which implies $\bar{f}(e_1)=-\bar{f}(e_2)$.
\end{proof}
...
Let $\mathbf{A}$ be an abelian group and let $G$ be a multigraph.  Let $\mathcal{P}=\{P_1,\ldots, P_k\}$ be a partition
...
to a single vertex $p_i$.  If $G$ has a nowhere-zero $\mathbf{A}$-flow, then $G'$ has a nowhere-zero $\mathbf{A}$-flow as well.
\end{corollary}
\begin{proof}
...
$f'$ is a nowhere-zero $\mathbf{A}$-flow in $G'$, since Observation~\ref{obs:flowoncut} applied to $P_i$ for $i=1,\ldots, k$
...
arbitrarily.  Corollary~\ref{cor:contractflow} then implies that it suffices to find a nowhere-zero $\mathbf{A}$-flow in the resulting
graph.

Converse oparation (contracting a set of vertices $S$ and deriving a nowhere-zero $\mathbf{A}$-flow in the original graph $G$ from the one
...
Let $\mathbf{A}$ be an abelian group and let $G$ be a multigraph.  Let $e$ be an edge of $G$ contained in a $2$-edge-cut.
Then $G$ has a nowhere-zero $\mathbf{A}$-flow if and only if $G/e$ does.
\end{lemma}
\begin{proof}
By Corollary~\ref{cor:contractflow}, if $G$ has a nowhere-zero $\mathbf{A}$-flow, then so does $G/e$.

Conversely, suppose $f$ is a nowhere-zero $\mathbf{A}$-flow in $G/e$.  Let $e=uv$ and let $e'=u'v'$ be the other edge of a $2$-edge-cut
containing $e$, labelled so that $u$ and $u'$ are in the same component $S$ of $G-\{e,e'\}$.  Let us extend $f$ to $G$
by setting $\bar{f}(e)=-\bar{f}(e')$, so that $\bar{f}(V(S))=0$.
Note that the flow conservation law holds at all vertices except possibly for $u$ and $v$.
Furthermore, since $\bar{f}(V(S))=0$ and the flow conservation law holds at all vertices of $V(S)$ except possibly for $u$,
we conclude analogously to Observation~\ref{obs:flowcol} that the flow conservation law holds also at $u$.  A symmetrical
argument shows that the flow conservation law holds at $v$.  Hence, $f$ is a nowhere-zero $\mathbf{A}$-flow in $G$.
\end{proof}
...
from $G$ by \emph{lifting the edges $e_1$ and $e_2$ at $v$}.  Clearly, if $G'$ contains a nowhere-zero $\mathbf{A}$-flow,
...
Let $\mathbf{A}=\mathbf{A}_1\times \cdots\times \mathbf{A}_k$ be an abelian group.
For $i=1,\ldots, k$, let $f_i$ be an $\mathbf{A}_i$-flow in a multigraph $G$.
Let $f(e)=(f_1(e),\ldots,f_k(e))$ for each edge $e\in E(G)$.  Then $f$ is an $\mathbf{A}$-flow in $G$,
...
Let $G$ be a connected multigraph and let $T$ be a spanning tree of $G$.  Let $\mathbf{A}$ be a non-trivial abelian group
and let $f$ be an arbitrary function assigning elements of $\mathbf{A}$ to directed edges of $E(G)\setminus E(T)$.
Then $f$ can be extended to a $\mathbf{A}$-flow in $G$.  In particular, $G$ has an $\mathbf{A}$-flow whose support
...
$\mathbf{A}$ to directed edges of $E(G)\setminus E(T)$, such that the flow conservation law holds at all vertices of $V(G)\setminus V(T)$.
Then $f$ can be extended to a $\mathbf{A}$-flow in $G$.
...
However, then $f$ also satisfies the flow conservation law at $v$ (analogously to Observation~\ref{obs:flowoncut} applied to $S=V(G)\setminus\{v\}$,
noting that $\bar{f}(S)$ is equal to $\bar{f}(\{v\})$).

If $|V(T)|\ge 2$, then $T$ has a leaf $u$ incident with a unique edge $e_0=uw\in E(T)$.  Let us set
$$f(e_0)=-\sum_{e=ux\in E(G), e\neq e_0} \bar{f}(e),$$
so that the flow conservation law is satisfied at $u$.  The claim then follows by induction hypothesis applied to $T-u$.
\end{proof}
...
well-known connectivity condition, which is a corollary of a stronger theorem of Nash-Williams.
...
to prove the 5-Flow Conjecture cubic graphs (see the proof of Theorem~\ref{thm:6flow} below for a more
...
Analogously to Lemma~\ref{lemma:z223col}, one can show that $G$ has a nowhere-zero $\mathbf{Z}_2^2$-flow if and only if $G-v$ is $3$-edge-colorable.
...
By Lemma~\ref{lemma:treecomplflow}, for $i\in\{1,2\}$ there exists a $\mathbf{Z}_2$-flow $f_i$ in $G_i$ whose support includes the complement of $T_i$.
Since $T_1$ and $T_2$ are edge-disjoint, the union of the supports of $f_1$ and $f_2$ is $E(G)$, and thus
$G$ has a nowhere-zero $\mathbf{Z}_2^2$-flow by Observation~\ref{obs:prodflow}.
\end{proof}
...
\end{theorem}
\begin{proof}
We prove by induction on the number of vertices of $G$ that $G$ has a nowhere-zero $\mathbf{Z}_2^3$-flow, the case $|V(G)|=1$ being trivial.
...
By the induction hypothesis, $G/e$ has a nowhere-zero $\mathbf{Z}_2^3$-flow, and thus $G$ has a nowhere-zero $\mathbf{Z}_2^3$-flow by
...
By Lemma~\ref{lemma:treecomplflow}, for $i\in\{1,2,3\}$ there exists a $\mathbf{Z}_2$-flow $f_i$ in $G_i$ whose support includes the complement of $T_i$.
Since no edge of $G$ belongs to all of the trees $T_1$, $T_2$, and $T_3$, the union of the supports of $f_1$, $f_2$, and $f_3$ is $E(G)$.
Consequently, $G$ has a nowhere-zero $\mathbf{Z}_2^3$-flow by Observation~\ref{obs:prodflow}.
\end{proof}
...
\end{theorem}
\begin{proof}
We will prove that $G$ has a nowhere-zero $\mathbf{Z}_2\times\mathbf{Z}_3$-flow.
...
Let $T$ be a spanning tree of $K_k\cup H_k$; note that $T$ is also a spanning tree of $G$.  By Lemma~\ref{lemma:treecomplflow}, $G$ has a $\mathbf{Z}_3$-flow $f_k$
whose support contains $E(G)\setminus E(T)$, and thus $f_k$ can be $0$ only on edges of $K_k\cup H_k$.  Let us now transform $f_k$ into a $\mathbf{Z}_3$-flow $f_1$
...
Consequently, the $\mathbf{Z}_3$-flow $f_1$ can only be zero on the edges of $K_k$.
Let $f_0$ be the $\mathbf{Z}_2$-flow in $G$ whose value is $1$ on edges of $K_k$ and zero everywhere else.
The union of the supports of $f_0$ and $f_1$ is $E(G)$, and thus $G$ has a nowhere-zero $\mathbf{Z}_2\times \mathbf{Z}_3$-flow by Observation~\ref{obs:prodflow}.
\end{proof}

\subsection{Weak $3$-flow conjecture}

It is much more difficult to prove existence of a $3$-flow subject to some edge-connectivity assumption, partially because
$\mathbf{Z}_3$ is not a product of non-trivial groups, which makes it impossible to use Observation~\ref{obs:prodflow}.
...
Let us remark that to prove the $3$-Flow Conjecture, it would be enough to shou that 5-edge-connected multigraphs have nowhere-zero 3-flows~\cite{weak53eq}.

Consider a nowhere-zero $\mathbf{Z}_3$ flow $f$ in a multigraph $G$. Clearly, we can select an orientation $\vec{G}$ of $G$
...
An orientation of $G$ is a \emph{$\beta$-orientation} it is consistent with $\beta$ at all vertices of $G$.  Hence, $G$ has a nowhere-zero $3$-flow if and only if it has a $\beta$-orientation
...
we have $\beta'(V(G)\setminus X)=\beta'(X)\in\{1,2\}$ depending on the parity of $\deg(X)$.
...
Before we prove this result, let us observe that it indeed implies that $6$-edge-connected graphs have nowhere-zero $3$-flows.
Indeed, they have the following stronger property (called \emph{$\mathbf{Z}_3$-connectivity}).
\begin{corollary}\label{cor:z3conn}
A $6$-edge-connected multigraph $G$ has a $\beta$-orientation for any boundary $\beta$.
\end{corollary}
\begin{proof}
Let $G'$ be the graph obtained from $G$ by adding an isolated vertex $z_0$ and setting $\beta(z_0)=0$.
Note that if $A\subsetneq G$ is non-empty, then $\deg(A)\ge 6$, since $G$ is $6$-edge-connected, and thus
...
we have $\deg(X)=\beta'(X)+4$. Let $G_1$ be the graph obtained from $G$ by contracting $X$ into a new vertex $x$,
let $G_2$ be the graph obtained from $G$ by contracting $V(G)\setminus Z$ into a new vertex $y_0$, and let
...
assume that $x$ and $y$ are joined by an edge $e$.  Note that if $\beta'(v)=3$, then $\beta(v)=0$ and $v$ has odd degree; hence, we can by symmetry assume
...
If $(G,\beta,z_0,\vec{D})$ is an almost minimal counterexample, then $\beta'(v)\in \{1,2\}$ for all $v\in V(G)\setminus \{z_0\}$.
...
Let $G_2$ be the graph obtained from $G$ by replacing $e$ by $2$ parallel edges,
