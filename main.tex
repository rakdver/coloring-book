...
or in the list coloring setting from Theorem~\ref{thm:5choosbook} or Theorem~\ref{thm:5choos-nonbook} (in fact, the argument is implicit in the proof of the
...
We will need the following easy fact about graphs embedded in the torus.

\begin{lemma}\label{lemma:torus-cut-dist}
Let $G$ be a graph embedded in the torus, and let $C$ be the shortest non-contractible cycle in $G$.
Let $G'$ be obtained from $G$ by cutting the torus along the cycle $C$, and let $C_1$ and $C_2$ be the cycles
in $G'$ created from $C$. Then the distance between $C_1$ and $C_2$ in $G'$ is at least $|C|/2$.
\end{lemma}
\begin{proof}
Let $P'$ be a shortest path between $C_1$ and $C_2$ in $G'$, and let $P$ be the corresponding walk in $G$.
Let $\ell$ denote the length of $P$.
Let $K$ be a cycle in $G$ consisting of the path $P$ and a shortest path between its endpoints in $C$.
We have $|K|\le \ell+|C|/2$.  Note that $K$ is non-contractible, and since $C$ is a shortest non-contractible
cycle in $G$, we have $|K|\ge |C|$.  It follows that $\ell\ge |C|/2$.
\end{proof}

We can now use recoloring argument to obtain the following result.

\begin{theorem}\label{thm:torquadr}
If $G$ is a quadrangulation of torus with edge-width at least $5$, then $G$ is $3$-colorable.
\end{theorem}
\begin{proof}
Let $C=v_1v_2\ldots v_k$ be the shortest non-contractible cycle in $G$; we have $k\ge 5$.
Let $G'$ be obtained from $G$ by cutting the torus along the cycle $C$, and let $C_1$ and $C_2$ be the cycles
in $G'$ created from $C$.  For $i\in\{1,2\}$, let $C_i=v_{1,i}v_{2,i}\ldots v_{k,i}$, where $v_{j,1}$ and $v_{j,2}$ are the
vertices created by cutting $v_j$ for $j=1,\ldots,k$.  If $k$ is even, then $G'$ is bipartite; let $\varphi'$ be the $2$-coloring of $G'$
such that $\varphi'(v_{1,1})=1$.  If $\varphi'(v_{1,2})=1$, this gives a $2$-coloring of $G$; hence, assume that $\varphi'(v_{1,2})=2$.
Let $X=\{v_{j,2}:\text{$j\in\{1,\ldots, k\}$ is odd}\}$ and $Y=\{v_{j,1}:\text{$j\in\{1,\ldots, k\}$ is even}\}$
Let $\varphi'_1$ be a $3$-coloring of $G'$ obtained from $\varphi'$ by giving color $1$ to vertices of $X$
and color $3$ to all their neighbors and the vertices of $Y$.  Note that neighbors of vertices of $X$ had all the same color in $\varphi'$,
and thus they form an independent set.  Furthermore, by Lemma~\ref{lemma:torus-cut-dist}, the distance between $C_1$ and $C_2$ in $G'$ is
at least three, and thus they are not adjacent to vertices of $Y$.  Consequently, $\varphi'_1$ is a proper $3$-coloring of $G'$
and $\varphi'_1(v_{j,1})=\varphi'_1(v_{j,2})$ for $j\in\{1,\ldots, k\}$.  Hence, this gives a proper $3$-coloring of $G$.

Now consider the case that $k$ is odd.
Let $P$ be a shortest path between $C_1$ and $C_2$ in $G'$;
by relabelling the vertices of $C$ if necessary, we can assume that $v_{1,1}$ is an endpoint of $P$.
Let $G''$ be the graph obtained from $G'$ by cutting along $P$; this splits $C_1$ into a path $C'_1=v'_{1,1}v_{2,1}\ldots v_{k,1}v''_{1,1}$,
and $C_2$ into a path $C'_2=v'_{t,2}v_{t+1,2}\ldots v''_{t,2}$.

Note that $G''$ is a plane graph, the boundary of its outer face consists of the paths $C'_1$, $C'_2$, and the two paths created from $P$
by cutting, and all other faces of $G''$ have length $4$.  In particular, $G''$ is bipartite.  Let $\varphi''$ be the $2$-coloring
of $G''$ such that $\varphi''(v'_1^1)=1$.  Let us remark that since $k$ is odd, we have $\varphi''(v''_1^1)=2$.
...
\end{proof}
...
The graph $C(\mathbf{Z}_{13}; 1,5)$ has a unique embedding in the torus whose edge-width is $5$, and thus the all quadrangulations
of torus with representativity at least $6$ are $3$-colorable.
