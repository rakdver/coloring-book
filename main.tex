...
this can only be forced by $G$ containing short non-contractible cycles.

In the proof of this claim, the following notions are useful.  A set $X$ of vertices of $G$ is \emph{$d$-isolated} if the distance
...
We say $H$ is \emph{maximal} if there exists no $r$-circular skeleton $H'$ of $G$ such that $H\subsetneq H'$.
...
For every $\gamma>1$, there exists positive integers $r$ and $c$ such that the following holds.
Let $\GG$ be a cutting-closed class of graphs hyperbolic with Cheeger constant $\gamma$.
Let $G\in\GG$ be a graph with a boundary-perpendicular $2$-cell drawing in a surface of genus $g$ with $k$ holes
and let $H$ be a maximal $r$-circular skeleton of $G$.  Then $H$ has less than $2k+3g$ non-cylindrical faces,
and for each face $f$ of $H$, considering the stencil $(H,f)$, we have
$|V(G_f)|\le c$ if $f$ is cylindrical and $|V(G_f)|\le 2\gamma|\partial G\cap \partial G_f|+c(k+g)$ if $f$ is non-cylindrical.
\end{lemma}
\begin{proof}
We prove the claim holds for $r=2d+1$, where $d$ is as in Corollary~\ref{cor:hyperew},
and $c=14\gamma(52d+31)$.

Let $\HH$ be the set cycles of $H$ disjoint from the boundary of $\Sigma$, and let $\HH_1$, \ldots, $\HH_m$
be the partition of $\HH$ into classes of homotopically equivalent cycles.  For $i=1,\ldots, m$,
if $|\HH_i|=1$, then let $\Sigma_i$ be the subset of $\Sigma$ consisting of the drawing of the cycle in $\HH_i$,
...
By Lemma~\ref{lemma:disjnonc}, we have $m+1<2k+3g$.

Consider a face $f$ of $H$.  By the maximality of $H$, every non-contractible cycle in $G_f$ disjoint from $\partial G_f$
has length greater than $r$.  Let $t$ denote the number of holes of the surface in which $G_f$ is drawn, and $g_f$ its
genus; clearly $t\le 2m<4k+6g$ and $g_f\le g$.
Since $\GG$ is cutting-closed, $G_f\in\GG$, and thus Corollary~\ref{cor:hyperew}
implies $|V(G_f)|\le 2\gamma((48d+29)(t+g_f)+|\partial G_f|)$.  Observe also that if $f$ has $t$ boundary walks,
then $|\partial G_f|\le |\partial G\cap \partial G_f|+2rt$ (the factor of two accounts for the possibility that the
boundary walk corresponds to a one-sided cycle).  Together these inequalities imply
\begin{align*}
|V(G_f)|&\le 2\gamma((48d+29)(t+g_f)+2rt+|\partial G\cap \partial G_f|)\\
&\le 2\gamma|\partial G\cap \partial G_f|+2\gamma(48d+2r+29)(t+g_f)\\
&\le 2\gamma|\partial G\cap \partial G_f|+c(k+g).
\end{align*}
In case $f$ is cylindrical, bound follows with $t=2$, $g_f=0$, and $\partial G\cap \partial G_f=\emptyset$.
\end{proof}
