...
\begin{theorem}[Dirac~\cite{dirac1957short},  Albertson and Hutchinson~\cite{albertson1979three}]
...
is tight in this special case.  This graph also plays a role in another context, concerning the size of the smallest nontrivial critical graphs.
...
We say that $G$ is \emph{loose} if $G$ has a $(k+1)$-coloring with all color classes of size at least two; and in particular, $|V(G)|\ge 2k$.
The following lemma is a mild strengthening of results of Gallai~\cite{gallai2} and Stehl{\'\i}k~\cite{stehlik}.
A \emph{complete join} of two graphs $G_1$ and $G_2$ is obtained from their dijoint union by adding all edges between $V(G_1)$ and $V(G_2)$.
A graph $G$ is \emph{hypo-matchable} if for every $v\in V(G)$, $G-v$ has a perfect matching.
