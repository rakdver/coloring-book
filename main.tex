...
If $e\in E(G)\setminus\{uv\}$, then $G-e$ has a $k$-coloring, and by permuting the colors, we can assume this coloring assigns color $\psi(w_A)$ to $u$ and $\psi(w_B)$ to $v$;
combining this coloring with $\psi$ gives a $k$-coloring of $F$.  If $e\in E(H)$, then $H-e$ has a $k$-coloring; let $\psi'$ be a $k$-coloring of $H_{w\to(A,B)}-e$ obtained from this
$k$-coloring of $H-e$ by giving both $w_A$ and $w_B$ the color of $w$.  Since $G$ is critical for $k$-coloring, $G-uv$ has a $k$-coloring $\psi''$, and since $G$ is not $k$-colorable,
we have $\psi''(u)=\psi''(v)$; by permuting the colors, we can ensure that $\psi''(u)=\psi'(w_A)$.  Hence, the combination of $\psi''$ and $\psi'$ gives a proper $k$-coloring
of $F-e$.
\end{proof}
...
Note that all $(k+1)$-Ore graphs are critical for $k$-coloring and other than $K_{k+1}$, they have vertex-cuts of size two; thus, indeed,
...
who proved a bound that assymptotically matches the density of $(k+1)$-Ore graphs using a newly developed potential method; we give more detail in Chapter~\ref{chap:potential}.
