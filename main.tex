...
$$|\partial G_f|<(4g+2k)|V(G)|/c+(4g+2k+1)|\partial G|+5g+3k+2c.$$
\end{lemma}
\begin{proof}
...
Note that every path in $T'$ ending in $v_0$ contains at most $(r_1-r_0)/2$ vertices of $V(G)\setminus V(K_0)$.
For each even $r$ such that $r_0<r<r_1$, either $V(G_{r})\cap \partial G\neq \emptyset$,
or $|V(G_r)|\ge c$.  Consequently, $|V(G)|\ge c((r_1-r_0)/2-1-|\partial G|)$, and $(r_1-r_0)/2\le |V(G)|/c+|\partial G|+1$.
...
the paths from the ends of $X$ to $v'$ in $T'$, implying that $|V(H')\cap (V(G)\setminus V(K_0))|\le 2g(|V(G)|/c+|\partial G|+1)$.
Let $H''$ be obtained from $H'$ by, for each hole of $\Sigma$, adding the cycle in $R'$ bounding the hole
and a path from $v'$ to this cycle in $T'$.  We have
$|V(H'')\cap (V(G)\setminus V(K_0))|\le (2g+k)(|V(G)|/c+|\partial G|+1)+|\partial G|$.
Note that every face of $H''$ is $2$-cell in $\Sigma$.  Let $H$ be obtained from $H''$
by iteratively deleting edges between pairs of distinct faces; then $H$ has exactly one face $f$
and this face is $2$-cell, and thus $(H,f)$ is a net perpendicular to $G$.  We have
$$|V(H)\cap V(G)|\le |V(H'')\cap V(G)|<(2g+k)(|V(G)|/c+|\partial G|+1)+|\partial G|+c.$$
Consider a vertex $v\in V(G)\cap V(H)$, and let $b=1$ if $v\in\partial G$ and $b=0$ otherwise.
Note that when cutting along the net $(H,f)$, every vertex $v\in V(G)\cap V(H)$ is split into $\deg_H(v)-b$ vertices. 
Consequently, using Euler's formula, we obtain
\begin{align*}
|\partial G_f|&=2|V(G)\cap V(H)|-\partial G+\sum_{v\in V(G)\cap V(H)} (\deg_H(v)-2)\\
&=2|V(G)\cap V(H)|-|\partial G|+2(|E(H)|-|V(H)|)\\
&\le 2|V(G)\cap V(H)|-|\partial G|+k+g-1\\
&<(4g+2k)|V(G)|/c+(4g+2k+1)|\partial G|+5g+3k+2c.
\end{align*}
\end{proof}
