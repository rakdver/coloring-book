...
if $G$ is $H$-critical for $k$-coloring.  We say the vertices not in the boundary are \emph{internal}.
...
For the discharging argument, we will need several simple reducible configurations,
depicted in Figure~\ref{fig:wtredu5}.

\begin{lemma}\label{lemma:wtredu5}
Let $G$ be a graph, and let $X\subset V(G)$ and a proper $5$-coloring $\varphi$ of $G-X$
satisfy one of the following conditions.
\begin{itemize}
\item $G[X]$ is an edge $uv$, where $\deg_G(u)=\deg_G(v)=5$, and two of the neighbors of $u$ in $G-X$ are
assigned the same color by $\varphi$, or
\item $G[X]$ is an edge $uv$, where $\deg_G(u)=5$, $\deg_G(v)=6$, and $u$ and $v$ have two common neighbors
$w$ and $z$ in $G-X$ and $\varphi(w)=\varphi(z)$, or
\item $G[X]$ consists of a star with center $v$ and rays $v_1$, $v_3$, and $v_5$,
all vertices of $X$ have degree $6$ in $G$, for $i\in \{1,3,5\}$, $v$, $v_i$, and $v_{i+2}$ have a common
neighbor $v_{i+1}$ in $G-X$ (where $v_7=v_1$), and $\varphi(v_2)=\varphi(v_4)=\varphi(v_6)$.
\end{itemize}
Then $\varphi$ extends to a $5$-coloring of $G$.
\end{lemma}
\begin{proof}
In the first two cases, we first greedily color $v$ and then $u$.  
In the last case, we greedily color $v_1$, $v_3$, $v_5$, and $v$.
\end{proof}
Of course, just a single vertex of degree five can be colored if it has two neighbors of the same color,
so the reader might wonder why we bother with the more complicated configurations discussed in
Lemma~\ref{lemma:wtredu5}.  The reason is that knowing a larger part of the graph is deleted in the
reduction will be needed to argue about the weight of the resulting critical subgraph.
...
for some sufficiently small $\varepsilon>0$ ($\varepsilon=...$ will suffice).
Clearly $|V(G)|\le |V(C)|+w(G)/\varepsilon$, and thus to bound the size of $G$,
it suffices to bound $w(G)$.

We aim to prove the following result.
\begin{theorem}\label{thm:wt5}
Let $\varepsilon=...$ and $\beta=...$.  Suppose $G$ is a graph with a normal drawing in the disk bounded by a cycle $C$.
...
\item each vertex of $C$ has degree at least three, and
\item each internal vertex $v$ has at most three neighbors $v_1$, \ldots, $v_k$ in $C$,
$v_1\ldots v_k$ is a subpath of $C$, and $vv_iv_{i+1}$ is a $3$-face for $i=1,\ldots,k-1$.
\end{itemize}
...
Finally, consider a vertex $v$ with neighbors $v_1$, \ldots, $v_k$ in $C$ in the clockwise order, where $k\ge 2$.
...
and $C$ is induced; hence, this would decrease the right-hand side of the inequality by at least $2-\varepsilon$,
giving $w(G)\le |C|+3\varepsilon-4-k\le |C|+3\varepsilon-6\le |C|-5-\beta$, which is a contradiction.
Therefore, $G$ is nicely arched.
\end{proof}

Next, we use the discharging method to show that any graph of sufficiently large weight contains one of the
reducible configurations.  
\begin{lemma}\label{lemma:hwredu}
Let $G$ be a connected graph with a normal nicely arched drawing in the disk bounded by an induced cycle $C$
and such that every internal vertex has degree at least five.
If $w(G)>|C|-6$, then $G$ contains one of the following configurations:
\begin{itemize}
\item[(C1)] Adjacent internal vertices $u$ and $v$, where $\deg(u)=5$, $\deg(v)=6$, and 
both faces incident with $uv$ have length three and are only incident with internal vertices.


\item[(C2)] Adjacent internal vertices $u$ and $v$ of degree five, such that denoting the
faces incident with $u$ but not the edge $uv$ $f_1$, $f_2$, and $f_3$ in order,
either $f_1$ and $f_2$ are both $3$-faces or at least one of them is a $4$-face.???
\item[(C3)] An internal vertex $v$ of degree six such that all incident faces have length three
and all adjacent vertices are internal and have degree six.
\end{itemize}
\end{lemma}
\begin{proof}
Give each vertex $v\in V(G)$ the initial charge $\initch(v)=\deg(v)-6$ and each face $f$
the initial charge $2|f|-6$.  Note that the sum of these charges is $-6-2|C|$
(if $G$ were drawn in the plane, the sum would be $-12$ by Lemma~\ref{lemma:initch}),
but since it is drawn in the disk, $C$ does not bound a face of $G$).

A vertex $u$ is \emph{heavy} if either $u\in V(C)$ and $\deg(u)\ge 4$, or $\deg(u)\ge 7$.
We now redistribute the charge according to the following rules.
\begin{itemize}
\item[(R1)] Suppose $uv\in E(G)$, $u$ is heavy and $v$ is internal.
Then $u$ sends to $v$
\begin{itemize}
\item $\tfrac{1}{5}(1+\varepsilon)$ if $\deg(v)=5$ and $u$ is internal,
\item $\tfrac{\deg(u)-2}{5}(1+\varepsilon)$ if $\deg(v)=5$ and $u\in V(C)$, and
\item $\varepsilon$ if $\deg(v)\ge 6$.
\end{itemize}
\item[(R2)] ...
\item[(R3)]
\end{itemize}
