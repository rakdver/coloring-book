...
Let us give an example.  Let $v_0v_1v_2v_3v_4$ be a subpath of a boundary of a face in a plane
...
Every $2$-connected plane graph $G$ without cycles of lengths $4$ to $8$ contains a vertex of degree at most two
or a clean tetrad.
\end{lemma}
\begin{proof}
If $G$ does not contain a triangle, then Euler's formula easily implies that $G$ contains a vertex of degree at most two.
...
Let $C$ be a cycle in $G$ of length at most $12$ such that the open disk $\Lambda$ bounded by $C$ is minimum.
Let $G_C$ be the subgraph of $G$ drawn in the closure of $\Lambda$.

Clearly, $G_C$ is a $2$-connected graph and $C$ is an induced cycle in $G_C$.
If $G_C$ contained neither a vertex of degree at most two not contained in $C$ nor
a tetrad disjoint from $C$, then we would 
assign charge to the vertices and faces of $G_C$, perform the discharging, and obtain a contradiction in the same way as in
the proof of Theorem~\ref{thm:no4to8}.  Hence, $G_C$ contains one of these configurations.
If $G_C$ contains a vertex of degree at most two not belonging to $C$, then this vertex has the same degree in $G$ as well.
...
from $v_0$ to $y$ of length at most $8$, and thus the path $v_0v_1v_2v_3y$ is contained in a cycle $K'_1$ of length at most $12$.
As in the proof of Lemma~\ref{lemma:no4to8-prop}, we argue that the open disk $\Lambda'_1$ bounded by any such cycle $K'_1$ contains either $x$ or $v_4$.
If $\Lambda'_1\subseteq\Lambda$, then since both edges $v_2x$ and $v_3v_4$ are contained in $\Lambda$
and exactly one of them is contained in $\Lambda'_1$, we have $\Lambda'_1\subsetneq\Lambda$; however, this contradicts the choice
of $C$.  Hence,
\begin{itemize}
\item[($\star$)] \emph{if $K'_1$ is cycle is $G$ of length at most $12$ such that $v_0v_1v_2v_3y$ is a subpath of $K'_1$,
then the open disk bounded by $K'_1$ is not a subset of $\Lambda$.}
\end{itemize}

Let $K_1$ be the shortest cycle in $G$ containing the path $v_0v_1v_2v_3y$.
...
No face of $\FF_1$ has length three,
as otherwise either $K_1$ would have a chord or $C$ would have a chord drawn in $\Lambda$, contradicting the choice
of $K_1$ or $C$.  Hence, all faces of $\FF_1$ have length at least $9$. 
Furthermore, by ($\star$), the faces whose boundary contains $Q_1$
have length at least $13$.  Hence, $\sum_{f\in \FF_1} |f|\ge 26+9(|\FF_1|-2)$.
On the other hand, since $K_1$ has at least one edge not contained in
of $\Lambda$ by ($\star$), we have $\sum_{f\in \FF_1} |f|\le |C|+2(|K_1|-1)\le 34$.
Since $|C|,|K_1|\le 12$, we conclude that $|\FF_1|=2$, and thus $K_1$ intersects $\Lambda$ only in $Q_1$.

Note that also $v_4v_3v_2v_1$ is a tetrad, and unless it is clean, letting $K_2$ be the shortest cycle in $G$
(of length at most $12$) containing the path $v_5v_4v_3v_2x$, we by symmetry conclude that $K_2$ intersects
$\Lambda$ only in a path $Q_2$ containing $v_5v_4v_3v_2x$.

Let $F$ be the graph consisting of $C$, $Q_1$, $Q_2$, and the edges $v_1x$ and $v_4y$.
Let $\FF$ denote the set of faces of $F$ contained in $\Lambda$.  Let $t$ denote the number of $3$-faces of $\FF$
distinct from $v_1v_2x$ and $v_3v_4y$.  Since $Q_1$ and $Q_2$ are induced paths
and $C$ is an induced cycle in $F$, each such $3$-face must have
an edge of $Q_1$, $Q_2$, and $C$ each, and thus $t\le 2$.
Hence, $\sum_{f\in \FF} |f|\ge 6+3t+9(|\FF|-2-t)$.  On the other hand, $Q_1$ and $Q_2$ intersect in the edge $v_2v_3$,
and thus denoting by $\ell_1$ and $\ell_2$ the lengths of $Q_1$ and $Q_2$, respectively, we have
$\sum_{f\in \FF} |f|\le |C|+2(\ell_1+\ell_2+1)\le 58$.
...
\end{proof}
...
For another application of the method using a more involved choice of minimal separating cycle, see Lemma~\ref{lemma:exredu}.

Of course, the proof of Lemma~\ref{lemma:no4to8-exred} is a bit more complicated than the argument using a precolored cycle.
...
only one of them applies (and the precolored cycle trick is often easier).  Why might the ``minimum cycle'' argument be
sometimes preferable?

Firstly, note that Lemma~\ref{lemma:no4to8-exred} is independent on the kind of coloring we use.  Indeed, in Chapter~\ref{chap:corresp},
we use this lemma to generalize Theorem~\ref{thm:no4to8} to list coloring, and a result like Lemma~\ref{lemma:no4to8-exred} might be
of interest also in non-coloring contexts.

Secondly, sometimes a precoloring extension result would be cumbersome to state or argue about.  For instance, suppose we consider
...
no vertex other than $w$ of degree at most two, a standard argument based on Euler's formula shows that $G_2$ has
a face of length at most $5$ bounded by a cycle $C_2$.  By the minimality of $G$, any $3$-coloring of $C_2$ extends
...
We will address this question in Chapter~\ref{chap:recolor}, Lemma~\ref{lemma:extend6}.
...
of a graph is the length of the shortest odd cycle (or $\infty$ if the graph is bipartite).
\begin{lemma}[Folding Lemma]\label{lemma:folding}
...
Suppose first that $P_1\cup P_2=K$, i.e., $v_4\in V(P_1)$.  Then $|P_1|+|P_2|>|K|$ and $|P_1|>|K|/2$.
If $|K|$ is even, then the walk consisting of $Q_1$ and the clockwise path in $K$ from $v_3$ to $v_1$
...
Hence, $P_1\cup P_2\neq K$, and $v_4$ is an interior vertex of $P'_1$.  By planarity, $C_1$ and $C_2$ intersect
...
of the pace of $C_1$.
\end{proof}
Folding Lemma is widely applicable not only for ordinary proper coloring, but
...
and thus Folding Lemma implies that a hypothetical minimal counterexample has all faces of length $5$.

Let us remark that Folding Lemma often makes it possible to improve girth bounds in coloring results
...
By Folding Lemma, we can assume that their faces have length $g$, and thus the discharging argument
will go through unmodified (a care need to be taken to check that all reductions also work in the odd girth
setting, though).  Since $g$ is odd, a graph of girth $g-1$ has odd girth at least $g$, and thus
in this way a result holds for planar graphs of girth $g$ can be cheaply transformed into a result
that holds for planar graphs of girth $g-1$.

We cannot eliminate non-facial cycles of length less than $G$ by Folding Lemma,
and we need to deal with those using one of the methods of Section~\ref{sec:precolface}.  Of course,
when Folding Lemma is applied in a graph with a precolored face, one needs to discuss the possibility
...
Using Folding Lemma and the reduction from Lemma~\ref{lemma:grgim-redu}, Gr\"otzsch' Theorem follows easily
...
Another possible choice in the proof of Gr\"otzsch' Theorem is to first reduce it to the girth 5 case by Folding Lemma and a precolored cycle
...
Excluding the trivial case that this is prevented by a chord of $C$ and up to a permutation of colors, there is exactly
one $3$-coloring that does not extend: the one that assigns vertices of $C$ colors $1,2,3,1,2,3$ in order.

\begin{lemma}\label{lemma:extend6}
Let $G$ be a plane triangle-free graph with the outer face bounded by an induced $6$-cycle $C=v_1\ldots v_6$ and all other
faces of length $4$.  Let $\psi$ be a $3$-coloring of $C$.  The coloring $\psi$ extends to a $3$-coloring of $G$
if and only if $\psi(v_1)\neq\psi(v_4)$ or $\psi(v_2)\psi(v_5)$ or $\psi(v_3)\psi(v_6)$.
\end{lemma}
\begin{proof}
If $\psi(v_1)\neq\psi(v_4)$ or $\psi(v_2)\psi(v_5)$ or $\psi(v_3)\psi(v_6)$, then $C$ contains a $2$-colored subpath of
three vertices; by symmetry, we can assume that $\psi(v_1)=\psi(v_3)=1$ and $\psi(v_2)=2$.
...
\end{proof}
