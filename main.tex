...
\end{itemize}
The demand function \emph{refines} a boundary $\beta$ if $\beta(v)=d(v)\bmod 3$ for each $v\in V(G)$.
Note that if $\beta(v)=0$ and $\deg(v)=4$, if a demand function $d$ refines $\beta$, then $d(v)=0$
...
$f(\ovl{e})=-f(\ovr{e})$ for every $e\in E(G)$ such that $\ovr{e}\in E(\vec{G})$, and
...
The equivalence of (b) and (c) follows from the max-flow min-cut theorem.
\end{proof}
...
\end{lemma}
\begin{proof}
Suppose first $G$ has a proper $\mathbb{Z}_3$-coloring $\varphi$ extending $\psi$.  Let $f$ be the nowhere-zero $\mathbb{Z}_3$-flow
...
and extending $\vec{C}^\star_\psi$.  By Lemma~\ref{lemma:z3ord}, there exists a $\beta$-orientaion $\vec{G}^\star$ of $G^\star$ extending
...
Let $G$ be a plane graph with the outer face bounded by $C$, and let $F^{(i)}$ be the set of non-outer faces of $G$.
A \emph{primal demand function} with respect to $\psi$ is a function $d:F^{(i)}\to\mathbb{Z}$ such that
\begin{itemize}
\item $d(F^{(i)})=-\omega_\psi(C)$, and
...
with respect to $\psi$ satisfying the following conditions.
\begin{itemize}
\item Every cycle $K$ intersecting $C$ in at most one vertex satisfies $|d(K)|\le |K|$.
\item Every path $P$ intersecting $C$ in exactly two vertices $v_1$ and $v_2$ and both subpaths $Q$ with
endpoints $v_1$ and $v_2$ satisfy $|\omega_\psi(Q)+d(P\cup Q)|\le |P|$.
\end{itemize}
\end{lemma}
\begin{proof}
% TODO: zkontrolovat zneni, opravit znamenko/orientaci v \omega_\psi(Q)+d(P\cup Q)
\end{proof}

\begin{corollary}\label{cor:facesum}
...
\end{corollary}

In particular, in the case all faces except for the outer one have length $4$,
the winding number of $\psi$ must necessarily be $0$, and
we obtain the following nice characterization.
