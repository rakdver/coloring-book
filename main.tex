...
Let $\mathbf{A}$ be an abelian group, let $D$ be a multigraph and let $\vec{D}$ be its arbitrary orientation.  
For a vertex $v\in V(G)$, let $E^-(v)$ and $E^+(v)$ denote the sets of edges of $\vec{D}$ entering and leaving $v$, respectively.
An \emph{$\mathbf{A}$-flow}
...
clearly gives a valid $\mathbf{A}$-flow with basically the same properties.  Thus, consider the symmetric orientation $\overleftrightarrow{D}$
of $D$, where each edge $e$ is replaced by two oppositely directed edges $\overleftarrow{e}$ and $\overrightarrow{e}$.
We can define an \emph{$\mathbf{A}$-flow} on $D$ as a function $f:E(\overleftrightarrow{D})\to\mathbf{A}$
such that $f(\overleftarrow{e})+f(\overrightarrow{e})=0$ for every $e\in E(D)$,
...
where elements of an abelian group $\mathbf{A}$ play the role of colors.  Let $D$ be the plane dual of $G$.
For an edge $e\in E(\overleftrightarrow{G})$ from a vertex $u$ to a vertex $v$, let $g_1$ and $g_2$ be the face of $G$ on the left and right of $e$
(from any point of $e$ looking in its direction), and let $e^\star$ denote the edge of $\overleftrightarrow{D}$
corresponding to $e$ and directed from the vertex corresponding to $g_1$ to the vertex corresponding to $g_2$.
In this case, let us define $f:E(\overleftrightarrow{D})\to \mathbf{A}$ by setting
...
we conclude that $f$ is a $\mathbf{A}$-flow in $D$.
...
\end{lemma}
\begin{proof}
For an edge $e\in E(\overleftrightarrow{G})$, let us define $\delta(e)=f(e^\star)$.  For any walk $W=w_0w_1\ldots w_m$, let $\delta(W)=\sum_{i=1}^m \delta(w_{i-1}w_i)$.
...
For any element $a$ of $\mathbf{A}$ and $x\in V(G)$, let $\varphi_a(x)=a+\delta(P_x)$.
...
Hence, there exist $|\mathbf{A}|$ proper colorings $\varphi_a$ of $G$ for $a\in |\mathbf{A}|$ whose derived $\mathbf{A}$-flow is $f$.
There are no other such colorings, since it is easy to see that a coloring whose whose derived $\mathbf{A}$-flow is $f$
...
a $k$-flow $f_1$ in $G$ such that $f_1(e)\bmod k=f_0(e)$ for every $e\in E(\overleftrightarrow{G})$.  In particular, if $f_0$ is nowhere-zero,
...
let us define $$f(S)=\sum_{e=uv\in E(\overleftrightarrow{G}),u\in S,v\not\in S} f(e).$$

\begin{observation}\label{obs:flowoncut}
Let $\mathbf{A}$ be an abelian group.  If $f$ is an $\mathbf{A}$-flow in a multigraph $G$, then $f(S)=0$ for all $S\subseteq V(G)$.
\end{observation}
\begin{proof}
Note that
$$f(S)=\sum_{u\in S}\sum_{e\in E^+(G)} f(e)=0,$$
...
plane graphs with almost all faces of length exactly $4$; the reason being that nowhere-zero $\mathbf{Z}_3$-flows
in $4$-regular graphs (their duals) degenerate into ordinary integer flows (bounded by $1$ on each edge), as stated more precisely
in the following lemma.  For generality, we state the result in terms of boundaries and consistent orientations,
which we introduced in Subsection~\ref{ssec:weak3flow}.  For an undirected multigraph $G$, a \emph{demand function} is any function $d:V(G)\to\mathbf{Z}$
such that $d(V(G))=0$, where $d(X)=\sum_{x\in X} d(x)$ for any set $X\subseteq V(G)$.  A \emph{$\mathbf{Z}$-flow (with unit capacities) satisfying demands $d$}
is a function $f:E(\overleftrightarrow(G))\to \{-1,0,1\}$ such that $f(\overleftarrow{e})+f(\overrightarrow{e})=0$ for the opposite orientations
$\overleftarrow{e}$ and $\overrightarrow{e}$ of each edge $e\in E(G)$, and such that
