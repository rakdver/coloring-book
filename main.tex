...
colorings.  This is clear for the local recoloring methods: if we can independently alter the coloring at many disjoint locations,
...
We say that an embedding of a graph in a given surface is locally planar if its representativity (or possibly edge-width) is large enough---larger
...
A commonly used technique to show such generalizations is to cut the graph along many disjoint homotopically equivalent cycles, color the resulting graph of smaller genus,
and alter the coloring inside the cylinder formed by these cycles to obtain a proper coloring of the original graph.  This is best explained on an example.
We will need the following easy fact about graphs embedded in the torus (which can be generalized to arbitrary surfaces).
...
If $G$ is a quadrangulation of torus with representativity at least $...$, then $G$ is $3$-colorable.
\end{theorem}
\begin{proof}
...
\end{proof}
