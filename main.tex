...
Somewhat surprisingly, much of this progess turns out to be due to improved bounds
...
deviation.  A graph commonly called \emph{$k$-critical} if its chromatic number is $k$, but every
...
of other kinds of coloring this notation becomes actively confusing.  For example, in the setting
...
with the same set of vertices.  Partially due to this fact, it is actually quite rare for the distinction between vertex-criticality
and criticality to make difference.

A graph is $1$-colorable if and only if it has any edges, and thus $K_2$ is the only critical graph for $1$-coloring.
...
repeatedly for various subgraphs or contractions of the input graph).

Let us return back to the study of critical graphs.  Table~\ref{table:knowncrit} summarizes our current state
of knowledge concerning vertex-critical graphs of various girths and bounded genus, with the entries resolved
by Corollary~\ref{cor:small-simple} and Theorem~\ref{thm:crittoalg} highlighted in green.
...
and by Kostochka and Stiebitz~\cite{kostochka1996excess}.  However, a breakthrough came in the work of Kostochka and Yancey~\cite{koyanore},
who proved a near-optimal bound using a newly developed potential method; we give more detail in Chapter~\ref{chap:potential}.
Nevertheless, for our current discussion, the bound from Theorem~\ref{thm:critdens} is sufficient; in combination with
Corollary~\ref{cor:mad}, it gives the following extension to Corollary~\ref{cor:small-simple}, covering the blue entries in Table~\ref{table:knowncrit}.
