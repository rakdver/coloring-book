...
Let $C$ be an induced cycle in $G$ of length at most $12$ such that the open disk $\Lambda$ bounded by $C$ is minimum.
Let $G_C$ be the subgraph of $G$ drawn in the closure of $\Lambda$.

Clearly, $G_C$ is a $2$-connected graph.
...
If $\Lambda_1\subseteq\Lambda$, then since both edges $v_2x$ and $v_3v_4$ are contained in $\Lambda$
and exactly one of them is contained in $\Lambda_1$, we have $\Lambda_1\subsetneq\Lambda$; however, this contradicts the choice
of $C$.  Hence, $K_1$ is not contained in the closure of $\Lambda$; and since $K_1$ is an induced cycle, we have $|C|\neq 3$,
and thus $|C|\ge 9$.

Let $Q'_1$ be the subpath of $K_1$ containing $v_0v_1v_2v_3y$ such that endvertices of $Q'_1$ belong to $C$
and the rest of $Q'_1$ is contained in $\Lambda$.
...
Consider a face $f\in \FF_1$ whose boundary contains $Q'_1$ and let $K$ be the cycle bounding $f$.
...
the choice of $C$.  This is a contradiction showing that both faces of $\FF_1$  whose boundary contains $Q'_1$
have length at least $13$.
Hence, $\sum_{f\in \FF_1} |f|\ge 26+9(|\FF_1|-2)$.
On the other hand, since $K_1$ has at least one edge not contained in the closure
of $\Lambda$ by ($\star$), we have $\sum_{f\in \FF_1} |f|\le |C|+2(|K_1|-1)\le 34$.
Since $|C|,|K_1|\le 12$, we conclude that $|\FF_1|=2$, and thus $K_1$ intersects $\Lambda$ only in $Q'_1$.

Let $\ell'_1$ denote the length of $Q'_1$.
...
such faces, and thus $18\le |C|+2(|K_1|-\ell'_1)$, and $\ell'_1\le |K_1|-(9-|C|/2)\le 9$.
Let $Q_1$ be a shortest path in the subgraph of $G$ drawn in the closure of $\Lambda$ that contains
$v_0v_1v_2v_3y$ and the endpoints of $Q_1$ belong to $V(C)$, and let $\ell_1$ denote the length of $Q_1$;
we have $\ell_1\le \ell'_1\le 9$.

Note that also $v_4v_3v_2v_1$ is a tetrad, and unless it is clean in $G$, we by symmetry conclude that
a shortest path $Q_2$ in the subgraph of $G$ drawn in the closure of $\Lambda$ that contains
$v_0v_1v_2v_3y$ and whose endpoints belong to $V(C)$ has length $\ell_2\le 9$.
Clearly, we can assume that in addition to the edge $v_2v_3$, the paths $Q_1$ and $Q_2$ intersect
at most in their segments ending in $C$, and thus the graph $F$ consisting of $C$, $Q_1$, $Q_2$, and the edges $v_1x$ and $v_4y$
has exactly $7$ faces (one bounded by $C$, two by triangles $v_1v_2x$ and $v_3v_4y$, and four other faces of length at least $9$).
...
\end{proof}
